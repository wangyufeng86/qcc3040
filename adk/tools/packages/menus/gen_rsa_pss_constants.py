...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser(
        description='Generate an RSA and PSS constants C source code file from a pemtodfukey DFU public key file')

    parser.add_argument(
        '-i', '--input',
        required=True,
        help='specifies the [path and] filename of the pemtodfukey DFU public key file')
    parser.add_argument(
        '-o', '--output',
        default=None,
        help='optionally specifies the path and/or filename of thee RSA and PSS constants C source code file')
...
def main(args):
    ...
    input_dirname, input_filename = os.path.split(os.path.normpath(parsed_args.input))
    if input_dirname == '':
    ...
    output_dirfile = os.path.abspath(os.path.join(output_dirname, output_filename))
    ...
    try:
        ...
        print("Error {} '{}' on input file {}\n".format(exception.errno, exception.strerror, input_dirfile))
        return False

    if len(InputLine) < 4:
        print("Error: Too few lines ({}) read from {}\n").format(len(InputLine, input_dirfile))
        return False
    ...
    try:
        ...
        print("Error {} '{}' on output file {}\n".format(exception.errno, exception.strerror, output_dirfile))
        return False
