...
chiptype_lookup_dir = {
    "qcc512x_qcc302x": "1283_qcc512x",
    "qcc512x_rom_v21": "1500_qcc512x",
    "qcc514x_qcc304x": "1579_qcc514x",
...
def search_project_for_chip_type(devkit_root, project, workspace_file):
    ...
    return memory_options_list if memory_options_list else None
...
def search_projects_for_chip_type(devkit_root, workspace_file):
    ...
    for project in ws_projects.values():
        ...
        if project.default_configuration.properties.get('TYPE') == 'device_config':
            # Try and get the chip type from the project
            return search_project_for_chip_type(devkit_root, project, workspace_file)
...
def main(args):
    ...
    if mem_option:
        ...
        dest = os.path.join(os.path.dirname(parsed_args.workspace), "filesystems", "user_selected_specific_memory_type.htf")
    ...
    sys.stdout.flush()
    print("File with selected memory type generated at: {}".format(dest))
    return 0
