...
class Project(object):
    ...
    def get_properties(self, config):
        configuration = self.configurations.get(config)
        if configuration:
            return configuration.properties
        else:
            return {}

    def get_cap_props(self, config, capability):
        configuration = self.configurations.get(config, None)
        if configuration:
            return configuration.capabilities.get(capability, None)
        return None
