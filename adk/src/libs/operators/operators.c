...
{
    uint16 id;
    uint16 coarse_factory_gain;
    uint16 fine_factory_gain;
...
void OperatorsAdaptiveAncSetFactoryGain(Operator op, unsigned coarse_factory_gain, unsigned fine_factory_gain)
{
    aanc_set_factory_gain_msg_t gain_msg;
    gain_msg.id = ADAPTIVE_ANC_SET_FACTORY_GAIN;
    gain_msg.coarse_factory_gain = (uint16)coarse_factory_gain;
    gain_msg.fine_factory_gain = (uint16)fine_factory_gain;
...
void OperatorsAdaptiveAncGetCalculatedGain(Operator op, get_params_data_t* get_params_data)
{  
    if (get_params_data)
    {
        get_params_data->standard_params[0].id = ADAPTIVE_ANC_GAIN_CALC_OFFSET;
        OperatorsStandardGetParameters(op, get_params_data);
    }    
}

void OperatorsAdaptiveAncGetWarmGain(Operator op, get_params_data_t* get_params_data)
{
    if (get_params_data)
    {
        get_params_data->standard_params[0].id = ADAPTIVE_ANC_FF_GAIN_OFFSET;
        OperatorsStandardGetParameters(op, get_params_data);
    }
}
