    ...
    switch (theGatt->state)
    {
        ...
        case gatt_state_initialised:
            switch(id)
            {
                case MESSAGE_BLUESTACK_ATT_PRIM:
                    switch(type)
                    {
                      ...
                      GATT_READ_BY_UUID))
                        ...
                        case ATT_SET_BREDR_LOCAL_MTU_CFM:
                            gattHandleAttSetBrEdrLocalMtuCfm(
                                (const ATT_SET_BREDR_LOCAL_MTU_CFM_T *)message
                                );
                        return;
                ...
                case GATT_INTERNAL_SET_BREDR_LOCAL_MTU_REQ:
                    gattHandleInternalSetBrEdrLocalMtuReq(
                        (const GATT_INTERNAL_SET_BREDR_LOCAL_MTU_REQ_T *)message
                       );
                return;
