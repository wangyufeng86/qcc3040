...
void GattManagerConnectAsCentral(const Task task,
                                      const typed_bdaddr *taddr,
                                      gatt_connection_type conn_type,
                                      bool conn_timeout)
{
    ...
    if (gattManagerDataGetConnectAsCentralHandler() != NULL)
    {
    ...
    {
        MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_CENTRAL);
        message->task = task;
        message->taddr = *taddr;
        message->conn_type = conn_type;
        message->conn_timeout = conn_timeout;
        MessageSendConditionally(gattManagerDataGetTask(),
                                 GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_CENTRAL,
                                 message, gattManagerDataGetAdvertisingRequestedFlag());
    }
    else
    {
        GattConnectRequest(gattManagerDataGetTask(), taddr, conn_type, conn_timeout);
        gattManagerDataSetConnectAsCentralHandler(task);
    }
}


static void cancelConnectAsCentralCfm(Task task, gatt_manager_status_t status)
{
    MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_CANCEL_CONNECT_AS_CENTRAL_CFM);
    message->status = status;
    MessageSend(task, GATT_MANAGER_CANCEL_CONNECT_AS_CENTRAL_CFM, message);
}


void GattManagerCancelConnectAsCentral(Task default_task)
{
    Task connection_task = gattManagerDataGetConnectAsCentralHandler();
    ...
    {
        cid = gattManagerDataGetConnectAsCentralRemoteCid();
        gattManagerDataSetConnectAsCentralHandler(NULL);
        ...
        else
        {
            ...
            GATT_MANAGER_DEBUG_INFO(("GM: GattManagerCancelConnectAsCentral may have been requested in gap before init"));
    ...
    if (connection_task)
    {
        cancelConnectAsCentralCfm(connection_task, request_status);
    }
    if (default_task != connection_task)
    {
        cancelConnectAsCentralCfm(default_task, request_status);
    }
}
...
void GattManagerConnectAsCentralInternal(const GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_CENTRAL_T *params)
{
    GATT_MANAGER_PANIC_NULL(params, ("GM: Connect T oRemote Server Internal params NULL!"));

    GattManagerConnectAsCentral(params->task, &params->taddr, params->conn_type, params->conn_timeout);
}

void gattManagerConnectAsCentralCfm(const GATT_CONNECT_CFM_T * cfm)
{
    MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_CONNECT_AS_CENTRAL_CFM);

    *message = *cfm;

    if(gattManagerDataGetConnectAsCentralHandler())
    {
        MessageSend(gattManagerDataGetConnectAsCentralHandler(),
                    GATT_MANAGER_CONNECT_AS_CENTRAL_CFM, message);
        gattManagerDataSetConnectAsCentralHandler(NULL);
    }
}


void gattManagerConnectAsCentralInitialising(uint16 cid)
{
    gattManagerDataSetConnectAsCentralRemoteCid(cid);
}
