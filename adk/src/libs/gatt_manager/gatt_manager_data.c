...
struct __gatt_manager_data
{
    ...
    gatt_manager_connecting_client_t    connect_as_peripheral;  /* When waiting for a client device to connect, this data keeps track of the connection */
    gatt_manager_connecting_server_t    connect_as_central;  /* When waiting for a server device connection, this data keeps track of the connection */
...
/*
 * GATT Manager functions for the handler task when connecting as central
 * *****************************************************************************/
void gattManagerDataSetConnectAsCentralHandler(Task conn_task)
{
    if (gattManagerDataIsInit())
    {
        gatt_manager_data->connect_as_central.task = conn_task;
        gatt_manager_data->connect_as_central.cid = INVALID_CID;
    }
}

void gattManagerDataSetConnectAsCentralRemoteCid(uint16 cid)
{
    if (gattManagerDataIsInit() && gatt_manager_data->connect_as_central.task)
    {
        gatt_manager_data->connect_as_central.cid = cid;
    }
}


Task gattManagerDataGetConnectAsCentralHandler(void)
{
    if (gattManagerDataIsInit())
    {
        return gatt_manager_data->connect_as_central.task;
    }

    return NULL;
}

uint16 gattManagerDataGetConnectAsCentralRemoteCid(void)
{
    if (gattManagerDataIsInit())
    {
        return gatt_manager_data->connect_as_central.cid;
    }

    return INVALID_CID;
}

Task * gattManagerDataGetPointerToConnectAsCentralHandler(void)
{
    if (gattManagerDataIsInit())
    {
        return &gatt_manager_data->connect_as_central.task;
...
/*
 * GATT Manager functions for connecting as peripheral
 * *****************************************************************************/
Task gattManagerDataGetConnectAsPeripheralTask(void)
{
    if (gattManagerDataIsInit())
    {
        return gatt_manager_data->connect_as_peripheral.task;
    }

    return NULL;
}

void gattManagerDataSetConnectAsPeripheralTask(Task task)
{
    if (gattManagerDataIsInit())
    {
        gatt_manager_data->connect_as_peripheral.task = task;
    }
}

uint16 gattManagerDataGetConnectAsPeripheralRemoteCid(void)
{
    if (gattManagerDataIsInit())
    {
        return gatt_manager_data->connect_as_peripheral.cid;
    }

    return 0;
}

void gattManagerDataSetConnectAsPeripheralRemoteCid(uint16 cid)
{
    if (gattManagerDataIsInit())
    {
        gatt_manager_data->connect_as_peripheral.cid = cid;
    }
}
