...
static bool connectAsPeripheralParamsValid(Task task, const typed_bdaddr *taddr,
...
static void connectAsPeripheralCfmFail(Task task)
{
    MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_CONNECT_AS_PERIPHERAL_CFM);
    message->status = gatt_status_failure;
    MessageSend(task, GATT_MANAGER_CONNECT_AS_PERIPHERAL_CFM, message);
}

static void connectAsPeripheralCfm(const GATT_CONNECT_CFM_T * cfm)
{
    MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_CONNECT_AS_PERIPHERAL_CFM);

    *message = *cfm;

    MessageSend(gattManagerDataGetConnectAsPeripheralTask(),
                GATT_MANAGER_CONNECT_AS_PERIPHERAL_CFM, message);
}

static void cancelConnectAsPeripheralCfm(Task task, gatt_manager_status_t status)
{
    MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_CANCEL_CONNECT_AS_PERIPHERAL_CFM);
    message->status = status;
    MessageSend(task, GATT_MANAGER_CANCEL_CONNECT_AS_PERIPHERAL_CFM, message);
}
...
{
    if(cfm->cid != gattManagerDataGetConnectAsPeripheralRemoteCid())
    {
...
static void gattConnectCfmSuccess(const GATT_CONNECT_CFM_T * cfm)
{
    ...
    {
        cancelConnectAsPeripheralCfm(gattManagerDataGetConnectAsPeripheralTask(),
                                     gatt_manager_status_failed);
    }

    gattManagerDataAdvertisingState_Idle();
    connectAsPeripheralCfm(cfm);
}
...
static void clearConnectAsPeripheralData(void)
{
    gattManagerDataSetConnectAsPeripheralTask(NULL);
    gattManagerDataSetConnectAsPeripheralRemoteCid(0);
}
...
void GattManagerConnectAsPeripheral(Task task,
                                    ...
                                    gatt_connection_type conn_type)
{
    ...
    if(!connectAsPeripheralParamsValid(task, taddr, conn_type))
    {
    ...
    {
        gattManagerDataSetConnectAsPeripheralTask(task);

        if(gattManagerDataGetConnectAsCentralHandler())
        {
            MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_PERIPHERAL);
            message->task = task;
            if(NULL != taddr)
            {
            ...
            MessageSendConditionallyOnTask(gattManagerDataGetTask(),
                                           GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_PERIPHERAL,
                                           message, gattManagerDataGetPointerToConnectAsCentralHandler());
    ...
    {
        connectAsPeripheralCfmFail(task);
    }
}
...
void GattManagerCancelConnectAsPeripheral(void)
{
    ...
    switch(gattManagerDataGetAdvertisingState())
    {
        ...
        case gatt_manager_advertising_state_advertising:
            ...
            GattDisconnectRequest(gattManagerDataGetConnectAsPeripheralRemoteCid());
            break;

        case gatt_manager_advertising_state_queued:
            if(MessageCancelAll(gattManagerDataGetTask(), GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_PERIPHERAL) > 0)
            {
                cancelConnectAsPeripheralCfm(gattManagerDataGetConnectAsPeripheralTask(),
                                             gatt_status_success);
                clearConnectAsPeripheralData();
                gattManagerDataAdvertisingState_Idle();
            }
            else
            {
        ...
        case gatt_manager_advertising_state_idle:
            cancelConnectAsPeripheralCfm(gattManagerDataGetApplicationTask(),
...
void gattManagerConnectAsPeripheralInternal(const GATT_MANAGER_INTERNAL_MSG_CONNECT_AS_PERIPHERAL_T *params)
{
    ...
    gattManagerDataAdvertisingState_Idle();
    GattManagerConnectAsPeripheral(params->task,
...
{
    MAKE_GATT_MANAGER_MESSAGE(GATT_MANAGER_CONNECT_IND);
    *message = *ind;
    MessageSend(gattManagerDataGetApplicationTask(), GATT_MANAGER_CONNECT_IND, message);
}
...
{
    gattManagerDataSetConnectAsPeripheralRemoteCid(cid);
