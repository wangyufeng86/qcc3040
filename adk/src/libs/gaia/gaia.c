...
GAIA_T *gaia = NULL;
static gaia_app_version_t gaia_app_version;
...
{
    GAIA_DEBUG(("GaiaSetDataEndpointMode mode: %d\n", mode));
...
{
    return gaia->data_endpoint_mode;
}
...
{
    uint8 mode = GaiaGetDataEndpointMode();
...
{
    bool is_in_correct_state = TRUE;
    gaia_transport *transport = gaia->transport;

    GAIA_DEBUG(("GaiaUpgradeConnect\n"));
    ...
    if (gaia->upgrade_transport == NULL)
    {
        ...
        if(gaia->upgrade_transport->type==gaia_transport_gatt)
        {
            ...
            GAIA_DEBUG(("GaiaUpgradeConnect, enhanced ATT Streams getting registered with cid-->%x\n",transport->state.gatt.cid));
            if(transport->state.gatt.src)
            {
                GAIA_DEBUG(("GaiaUpgradeConnect, ATT Stream Source valid\n"));
                ...
                GAIA_DEBUG(("GaiaUpgradeConnect, GAIA Task registered for recieving the messages\n"));
            ...
            else
            {
                ...
                GAIA_DEBUG(("GaiaUpgradeConnect, ATT Streams were not configured.\n"));
            }
        }
    }
    else
    {
        GAIA_DEBUG(("GaiaUpgradeConnect, incorrect state\n"));
        is_in_correct_state = FALSE;
    }

    return is_in_correct_state;
}


/*************************************************************************
NAME
    GaiaUpgradeDisconnect
...
bool GaiaUpgradeDisconnect(void)
{
    ...
    GAIA_DEBUG(("GaiaUpgradeDisconnect\n"));
    ...
    {
        GAIA_DEBUG(("GaiaUpgradeDisconnect, incorrect state\n"));
        is_in_correct_state = FALSE;
    }
...
static void getNextUpgradeData(gaia_transport *transport)
{
    ...
    /*
     * Check if a message (partial or complete) can be queued for processing.
     * To prevent running out of memory pools owing to queued messages, flow-off
     * to prevent message drained out of the Source buffer for processing.
     * Source buffer is later drained out when its ideal in other words
     * always limit the messages that can be queued and outstanding to be
     * processed.
     */
    if (FALSE == UpgradeFlowControlProcessDataRequest(upgrade_data_buffer,
                                        upgrade_len + GAIA_UPGRADE_HEADER_SIZE))
        UpgradeFlowOffProcessDataRequest(TRUE); /* Flowed off */

    free(upgrade_data_buffer);

    /* If not flowed off, drop the message as its queued for processing. */
    if(FALSE == UpgradeIsProcessDataRequestFlowedOff())
...
static bool isLargeDataUpgrade(gaia_transport *transport, uint8 *payload, uint8 payload_length)
{
    ...
    {
        GAIA_DEBUG(("GaiaUpgradeControl isLargeDataUpgrade, transport not the right type\n"));
        return FALSE;
    }

    if(payload_length <= (UPGRADE_MAX_PARTITION_DATA_BLOCK_SIZE + GAIA_UPGRADE_HEADER_SIZE))
    {
        GAIA_DEBUG(("GaiaUpgradeControl isLargeDataUpgrade not the right size\n"));
        return FALSE;
    }

    if(payload[GAIA_UPGRADE_COMMAND_ID_OFFSET] != GAIA_UPGRADE_HOST_DATA_REQ)
    {
        GAIA_DEBUG(("GaiaUpgradeControl isLargeDataUpgrade, not upgrade host data request\n"));
        return FALSE;
    }
...
bool GaiaUpgradeControl(uint8 *payload, uint8 payload_length)
{
    ...
    if (transport == gaia->upgrade_transport)
    {
        ...
        {
            GAIA_DEBUG(("GaiaUpgradeControl, NOT large data transport\n"));
            /*
             * Check if a message (partial or complete) can be queued for
             * processing. To prevent running out of memory pools owing to
             * queued messages, flow-off to prevent message drained out of the
             * Source buffer for processing.
             * Source buffer is later drained out when its ideal in other words
             * always limit the messages that can be queued and outstanding to
             * be processed.
             */
            if (FALSE == UpgradeFlowControlProcessDataRequest(payload,
                                                                payload_length))
                UpgradeFlowOffProcessDataRequest(TRUE); /* Flowed off */
        }
    }
    ...
    {
        GAIA_DEBUG(("GaiaUpgradeControl, incorrect state\n"));
        is_in_correct_state = FALSE;
    }
    ...
    uint16 command_id, uint8 payload_length, uint8 *payload)
{
    ...
    */
    ...
    case GAIA_COMMAND_VM_UPGRADE_CONTROL:
        ...
        if (!GaiaUpgradeControl(payload, payload_length))
        {
...
void handle_upgrade_transport_data_cfm(uint8 packet_type, upgrade_status_t status)
{
    ...
    if (gaia_app_version == gaia_app_version_2)
    {
        GAIA_DEBUG(("GAIA: UPGRADE_PROCESS_DATA_CFM v2 App"));
        send_ack(gaia->upgrade_transport, GAIA_VENDOR_QTIL, GAIA_COMMAND_VM_UPGRADE_CONTROL, status, 0, NULL);
    }
    else if (gaia_app_version == gaia_app_version_3)
    {
        GAIA_DEBUG(("GAIA: UPGRADE_PROCESS_DATA_CFM v3 App"));
        uint8 payload = (uint8)status;
        gaiaProcessCommand(gaia->transport, GAIA_V3_VENDOR_ID, GAIA_COMMAND_VM_UPGRADE_FW_DATA_CONFIRM_TO_APP, sizeof(payload), &payload);
    }
    else
    {
        GAIA_DEBUG(("GAIA: UPGRADE_PROCESS_DATA_CFM\n"));
 ...
 */
static bool handle_upgrade_transport_data_ind(bool is_data_state, uint8* data, uint16 size_data)
{
...
*/
    ...
    case UPGRADE_TRANSPORT_DATA_IND:
        ...
        if(gaia->upgrade_transport)
        {
            GAIA_DEBUG(("GAIA: UPGRADE_TRANSPORT_DATA_IND notify"));

            UPGRADE_TRANSPORT_DATA_IND_T *ind = (UPGRADE_TRANSPORT_DATA_IND_T *) message;
            if(handle_upgrade_transport_data_ind(ind->is_data_state, ind->data, ind->size_data))
            {
                if ((gaia_app_version == gaia_app_version_2) && (gaia->upgrade_transport->notify_vmup))
                {
                    GAIA_DEBUG(("GAIA: UPGRADE_TRANSPORT_DATA_IND v2 App"));
                    send_notification(gaia->upgrade_transport, GAIA_EVENT_VMUP_PACKET, ind->size_data, ind->data);
                }
                else
                {
                    GAIA_DEBUG(("GAIA: UPGRADE_TRANSPORT_DATA_IND v3 App"));
                    gaiaProcessCommand(gaia->transport, GAIA_V3_VENDOR_ID, GAIA_COMMAND_VM_UPGRADE_FW_NOTIFICATION_TO_APP, ind->size_data, ind->data);
                }
            }
        }
        break;
    ...
    case GAIA_INTERNAL_SEND_REQ:
        {
            GAIA_DEBUG(("gaia message_handler GAIA_INTERNAL_SEND_REQ\n"));
...
void gaiaProcessCommand(gaia_transport *transport, uint16 vendor_id, uint16 command_id, uint16 size_payload, uint8 *payload)
{
    ...
    else
    {
        ...
        if ((gaia_app_version == gaia_app_version_2) && (vendor_id == GAIA_VENDOR_QTIL))
        {
        ...
        /*  Pass it on sans the start-of-frame and checksum bytes  */
            ...
            if (gunk)
            {
                ...
                if (gaia_app_version == gaia_app_version_2)
                {
                    GAIA_CMD_DEBUG(("gaiaProcessCommand send to V2 App\n"));
                    MessageSend(gaia->app_task, GAIA_UNHANDLED_COMMAND_IND, gunk);
                }
                else if (gaia_app_version == gaia_app_version_3)
                {
                    GAIA_CMD_DEBUG(("gaiaProcessCommand send to V3 App\n"));
                    gaia->gaia_v3_command_handler(gaia->app_task, gunk);
                }
                else
                {
                    GAIA_CMD_DEBUG(("gaiaProcessCommand tying to send to unknown App\n"));
                }
...
void GaiaInit(Task task, uint16 max_connections)
{
    ...
    gaia_app_version = gaia_app_version_2;
}
                           ...
                           uint8 size_payload, uint8 *payload)
{
    ...
    case gaia_transport_gatt:
        GAIA_TRANS_DEBUG(("GaiaBuildAndSendSynch GAIA_TRANSPORT_GATT\n"));
                             ...
                             uint16 size_payload, uint16 *payload)
{
    ...
    {
        GAIA_TRANS_DEBUG(("GaiaBuildAndSendSynch16 GAIA_TRANSPORT_GATT\n"));
