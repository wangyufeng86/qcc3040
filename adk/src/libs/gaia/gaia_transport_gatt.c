...
void gaiaTransportGattRes(gaia_transport *transport, uint16 size_response, uint8 *response, uint8 handle)
{
    if(transport != NULL)
    {
        ...
        {
            GAIA_TRANS_DEBUG(("gaiaTransportGattRes handle == HANDLE_GAIA_DATA_ENDPOINT\n"));
        ...
        {
            GAIA_TRANS_DEBUG(("gaiaTransportGattRes size_response >= GAIA_GATT_OFFS_PAYLOAD\n"));
            ...
            if ((response[GAIA_GATT_OFFS_COMMAND_ID_H] & GAIA_ACK_MASK_H) || (GaiaGetAppVersion() != gaia_app_version_2))
            {
                           ...
                           bool unpack)
{
    ...
    GAIA_TRANS_DEBUG(("gaiaTransportGattSend\n"));
...
void gaiaTransportGattSendPacket(Task task, gaia_transport *transport, uint16 length, uint8 *data)
{
    ...
    GAIA_TRANS_DEBUG(("gaiaTransportGattSendPacket\n"));
...
bool gaiaTransportGattProcessSource(void)
{
    ...
    if(packetSize)
    {
        ...
        {
            /* If not flowed off, drop message as its queued for processing. */
