...
\code
LedConfigure(LED_0, LED_ENABLE, 1);  Turn on LED 0 pad

LedConfigure(LED_0, LED_ENABLE, 0);  Turn off LED 0 pad
\endcode
     */
    LED_ENABLE,
...
Assuming a 32MHz clock pwm_period is calculated as follows:
    ...
    pwm_period = (LED_PERIOD + 1) * ~128ms

which gives a maximum period of ~2.05s.
     */
    LED_PERIOD,
...
will be:
    ...
    /*!
Sets hold time for max PWM when flashing. This is represented in units of ~16ms
assuming a 32MHz clock.
     */
    LED_FLASH_MAX_HOLD,

    /*!
Sets hold time for min PWM when flashing. This is represented in units of ~16ms
assuming a 32MHz clock.
     */
    LED_FLASH_MIN_HOLD,
