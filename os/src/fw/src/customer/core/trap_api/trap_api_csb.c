...
Source StreamCsbSource(const bdaddr * remote_addr, uint16 lt_addr)
{
    ...
    ipc_transaction(IPC_SIGNAL_ID_STREAM_CSB_SOURCE, &ipc_send_prim, sizeof(ipc_send_prim),
                    IPC_SIGNAL_ID_STREAM_CSB_SOURCE_RSP, &ipc_recv_prim);
...
bool CsbReceiverSetAfhMap(const bdaddr * remote_addr, uint16 lt_addr, AfhMap * afh_map, uint32 afh_instant)
{
    ...
    ipc_transaction(IPC_SIGNAL_ID_CSB_RECEIVER_SET_AFH_MAP, &ipc_send_prim, sizeof(ipc_send_prim),
                    IPC_SIGNAL_ID_CSB_RECEIVER_SET_AFH_MAP_RSP, &ipc_recv_prim);
