...
/* Maximum and minimum decimation factor */
#define USB_AUDIO_RX_MAX_DECIM_FACTOT 10
#define USB_AUDIO_RX_MIN_DECIM_FACTOT 1
...
typedef struct
{
    ...
    /* USB Audio Rx normally is kicked and runs every 1ms and
     * in each run will generate a metadata tag. This is a
     * decimation factor, currently only used
     * to slow down the rate at which metadata are generated
     * but might be used in future to also run the operator
     * less oftem.
     * Note: This is set by user, if not set effective value
     * of 1ms will be used.
     */
     unsigned usb_audio_rx_decim_ms;
    ...
    /* minimum metadata tag lemgth in samples - Rx only */
    unsigned metadata_min_tag_len;

    /* samples left to complete last written metadata tag - Rx only */
    unsigned metadata_tag_left_words;
...
extern bool usb_audio_opmsg_set_run_decim_factor(OPERATOR_DATA *op_data, void *message_data,
