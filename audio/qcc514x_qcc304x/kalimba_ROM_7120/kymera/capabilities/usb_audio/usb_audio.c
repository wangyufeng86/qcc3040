...
const opmsg_handler_lookup_table_entry usb_audio_opmsg_handler_table[] =
{
    ...
    {OPMSG_USB_AUDIO_ID_SET_RUN_DECIM_FACTOR, usb_audio_opmsg_set_run_decim_factor},
...
MAP_INSTANCE_DATA(CAP_ID_USB_AUDIO_RX, USB_AUDIO_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_USB_AUDIO_RX, USB_AUDIO_OP_DATA)
...
#if !defined(CAPABILITY_DOWNLOAD_BUILD)
MAP_INSTANCE_DATA(CAP_ID_USB_AUDIO_TX, USB_AUDIO_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_USB_AUDIO_TX, USB_AUDIO_OP_DATA)
#endif /* CAPABILITY_DOWNLOAD_BUILD */

static inline USB_AUDIO_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (USB_AUDIO_OP_DATA *) base_op_get_instance_data(op_data);
}

/* Function check if one of the pcm are connected. */
static bool usb_audio_pcm_connected(USB_AUDIO_OP_DATA *usb_audio_data)
{

    unsigned i;
    for (i = 0; i < USB_AUDIO_OP_MAX_CHANNELS; i++)
    {
        if (usb_audio_data->pcm_channel_buf[i] != NULL)
        {
            return TRUE;
        }
    }
...
{
    USB_AUDIO_OP_DATA *usb_audio_data = get_instance_data(op_data);
    unsigned terminal_id = OPMGR_GET_OP_DATA_FORMAT_TERMINAL_ID(message_data);
                                           ...
                                           unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    usb_audio_data->data_format = (USB_AUDIO_PACKET_DATA_FORMAT)OPMSG_FIELD_GET(message_data, OPMSG_USB_AUDIO_SET_CONNECTION_CONFIG, DATA_FORMAT);
    ...
    {
        /* set minimum tag len in samples */
        unsigned min_tag_len_ms = MAX(1, usb_audio_data->usb_audio_rx_decim_ms);
        usb_audio_data->metadata_min_tag_len = min_tag_len_ms * frac_mult(usb_audio_data->sample_rate,
                                                              FRACTIONAL(0.001));
...
/**
 * usb_audio_opmsg_set_run_decim_factor
 * \brief message handler for OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR message
 *
 * \param op_data Pointer to the operator instance data.
 * \param message_data Pointer to the start request message
 * \param resp_length pointer to location to write the response message length
 ...
 * response
 */
bool usb_audio_opmsg_set_run_decim_factor(OPERATOR_DATA *op_data, void *message_data,
                                          unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    USB_AUDIO_OP_DATA *usb_audio_data = get_instance_data(op_data);
    unsigned decim_factor;

    /* only allow this for Rx operator and before configuring coonetion params
     * (the latter also (implicitly) means op isn't running so we don't check that)
     */
    if (USB_AUDIO_IS_TX_OPERATOR(usb_audio_data) || usb_audio_data->op_configured)
    {
        return FALSE;
    }

    /* read the decim factor */
    decim_factor = OPMSG_FIELD_GET(message_data,
                                   OPMSG_USB_AUDIO_SET_RUN_DECIM_FACTOR,
                                   DECIM_FACTOR_MS);
    /* Check the limits */
    if(decim_factor < USB_AUDIO_RX_MIN_DECIM_FACTOT ||
       decim_factor > USB_AUDIO_RX_MAX_DECIM_FACTOT)
    {
        return FALSE;
    }

    /* set the decim factor */
    usb_audio_data->usb_audio_rx_decim_ms = decim_factor;
    L2_DBG_MSG1("USB Audio Rx Operator, decim factor set: %d", usb_audio_data->usb_audio_rx_decim_ms);
    return TRUE;
}
                      ...
                      unsigned *response_id, void **response_data)
{
    ...
    if (base_op_get_cap_id(op_data) == USB_AUDIO_TX_CAP_ID)
    {
        usb_audio_data->is_usb_audio_tx = TRUE;
    }

#ifdef USB_AUDIO_SUPPORT_METADATA
    if (base_op_get_cap_id(op_data) == USB_AUDIO_RX_CAP_ID)
    {
                       ...
                       unsigned *response_id, void **response_data)
{
    ...
    else
    {
        if(bufp == &usb_audio_data->usb_audio_buf)
        {
             ...
             */
            usb_audio_data->usb_audio_ep =  stream_get_connected_endpoint_from_terminal_id(base_op_get_int_op_id(op_data), terminal_id);
        ...
        /* connect the buffer */
        *bufp = OPMGR_GET_OP_CONNECT_BUFFER(message_data);
#ifdef USB_AUDIO_SUPPORT_METADATA
        ...
        {
            /* create scratch cbuffer structure for this channel of tx operator,
             * they aren't used if the operator doesn't do rate adjustment
             * but we create them unconditionally here since still we don't
             * know whether it will do rate adjustment, the structure will be
             * deleted when the channel is disconnected.
             */
                           ...
                           unsigned *response_id, void **response_data)
{
    ...
    else if (*bufp)
    {
        ...
        if(usb_audio_data->pcm_metadata_buffer == *bufp)
        {
            ...
            if(USB_AUDIO_IS_RX_OPERATOR(usb_audio_data) &&
               NULL == new_metadata_buf)
            {
                /* metadata buffer isn't valid anymore,
                 * reset metadata_tag_left_words in case
                 * it's reconnected later.
                 */
                usb_audio_data->metadata_tag_left_words = 0;
            }
                                     ...
                                     unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (opmgr_op_is_running(op_data))
    {
        L2_DBG_MSG("usb_audio_opmsg_set_buffer_size: Cannot set buffer size because the operator is running!");
        return FALSE;
    }

    if  (usb_audio_pcm_connected(usb_audio_data))
    {
        L2_DBG_MSG("usb_audio_opmsg_set_buffer_size: Cannot set the buffer size for an operator with connected pcm!");
        return FALSE;
    }
...
void usb_audio_rx_generate_metadata_with_ttp(USB_AUDIO_OP_DATA *usb_audio_data, unsigned samples)
{
    ...
    if(usb_audio_data->metadata_tag_left_words > 0)
    {
        /* last written tag was incomplete, we keep adding
         * Null tag until full length of incomplete tag is
         * covered.
         */
        unsigned null_tag_len = usb_audio_data->metadata_tag_left_words;
        ...
        buff_metadata_append(usb_audio_data->pcm_metadata_buffer, NULL, b4idx, afteridx);

        /* update amount left */
        usb_audio_data->metadata_tag_left_words -= null_tag_len;
    ...
    if (mtag != NULL)
    {
        ...
        if(samples >= usb_audio_data->metadata_min_tag_len)
        {
        ...
        else
        {
             ...
             */
            mtag->length = usb_audio_data->metadata_min_tag_len*OCTETS_PER_SAMPLE;
            usb_audio_data->metadata_tag_left_words = usb_audio_data->metadata_min_tag_len - samples;
        }

        /* calculate time to play */
        ttp_update_ttp(usb_audio_data->time_to_play, time_get_time(), mtag->length/OCTETS_PER_SAMPLE, &status);
...
void usb_audio_process_data(OPERATOR_DATA * op_data, TOUCHED_TERMINALS *touched)
{
    ...
    if(usb_audio_data->data_format == UADF_T1_PCM)
    {
        ...
        if(USB_AUDIO_IS_TX_OPERATOR(usb_audio_data))
        {
            ...
            if ((usb_audio_data->pcm_metadata_buffer != NULL) && buff_has_metadata(usb_audio_data->pcm_metadata_buffer))
            {
...
{
    USB_AUDIO_OP_DATA *op_extra_data = get_instance_data(op_data);
    ...
    if(resp)
    {
        resp = cpsPack2Words(op_extra_data->sample_rate, op_extra_data->nrof_channels, resp);
        resp = cpsPack2Words(op_extra_data->data_format, op_extra_data->bit_resolution, resp);
        cpsPack1Word(op_extra_data->is_usb_audio_tx, resp);
    }
