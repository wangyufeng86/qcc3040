...
MAP_INSTANCE_DATA(CAP_ID_SPDIF_DECODE, SPDIF_DECODE_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_SPDIF_DECODE, SPDIF_DECODE_OP_DATA)
#endif /* CAPABILITY_DOWNLOAD_BUILD */

static inline SPDIF_DECODE_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (SPDIF_DECODE_OP_DATA *) base_op_get_instance_data(op_data);
}
...
bool spdif_decode_create(OPERATOR_DATA * op_data, void *message_data,
                         unsigned *response_id, void **response_data)
{
    SPDIF_DECODE_OP_DATA *spdif_decode_data = get_instance_data(op_data);
                         ...
                         unsigned *response_id, void **response_data)
{
    ...
    /* store connected spdif endpoint (only for efficiency) */
    spdif_decode_data->spdif_ep =
        stream_get_connected_endpoint_from_terminal_id(base_op_get_int_op_id(op_data),
                                                       SPDIF_DECODE_SINK_EP_TERMINAL|TERMINAL_SINK_MASK);
    return TRUE;
}
                                  ...
                                  unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    L2_DBG_MSG2("SPDIF_DECODE_OP, set output rate message from to client: time=%08x,  rate=%d",
                time_get_time(), spdif_decode_data -> output_sample_rate);
                                        ...
                                        OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    /* Note: This message is expected to be the response to
    ...
    spdif_decode_data -> current_decoder_data_type = (SPDIF_DATA_TYPE)OPMSG_FIELD_GET(message_data,
    ...
    L3_DBG_MSG2("SPDIF_DECODE_OP, set decoder data type message from to client: time=%08x,  type=%d",
                time_get_time(), spdif_decode_data -> current_decoder_data_type);
                                           ...
                                           OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    L3_DBG_MSG3("SPDIF_DECODE_OP, set supported data data types message from to client: time=%08x,  type=%04x,%0x4x",
                time_get_time(),
                OPMSG_FIELD_GET(message_data, OPMSG_SPDIF_DECODE_SET_SUPPORTED_DATA_TYPES, SUPPORTED_DATA_TYPES_GROUP1),
                OPMSG_FIELD_GET(message_data, OPMSG_SPDIF_DECODE_SET_SUPPORTED_DATA_TYPES, SUPPORTED_DATA_TYPES_GROUP2));
...
void spdif_decode_send_message_to_the_client(OPERATOR_DATA *op_data, unsigned msg_id, unsigned length, const unsigned *payload)
{
    if(!common_send_unsolicited_message(op_data, msg_id, length, payload))
    {
         ...
         */
        L3_DBG_MSG4("SPDIF_DECODE_OP, Sending Message to client FAILED: time=%08x,  id=%04x, data1=%4x, data2=%04x", time_get_time(), msg_id,
                    length>0?payload[0]:0xFFFF, length>1?payload[1]:0xFFFF);

    }

    L3_DBG_MSG4("SPDIF_DECODE_OP, Message sent to client: time=%08x,  id=%04x, data1=%4x, data2=%04x", time_get_time(), msg_id,
                length>0?payload[0]:0xFFFF, length>1?payload[1]:0xFFFF);
}
...
static bool spdif_decode_report_sample_rate_change(OPERATOR_DATA * op_data)
{
    ...
    if(!stream_get_sample_rate(spdif_decode_data->spdif_ep, &val32))
    {
        val32 = 0;
    }
        ...
        spdif_decode_data->output_sample_rate)
    {
        ...
        spdif_decode_data->timeout_start_time = time_get_time();
        return TRUE;
    }
    return FALSE;
}
