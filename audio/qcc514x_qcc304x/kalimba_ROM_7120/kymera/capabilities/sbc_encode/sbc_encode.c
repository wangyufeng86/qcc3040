...
MAP_INSTANCE_DATA(CAP_ID_SBC_ENCODER, SBC_ENC_OP_DATA)
#else
MAP_INSTANCE_DATA(CAP_ID_DOWNLOAD_SBC_ENCODER, SBC_ENC_OP_DATA)
...
*/
static inline SBC_ENC_OP_DATA *get_instance_data(OPERATOR_DATA *op_data)
{
    return (SBC_ENC_OP_DATA *) base_op_get_instance_data(op_data);
}
...
{
    SBC_ENC_OP_DATA *sbc_data = get_instance_data(op_data);
                                ...
                                unsigned *response_id, void **response_data)
{
    ...
    if (!mem_table_zalloc_shared((void *)(&(sbc_data->codec_data)),
                    ...
                    &new_allocation))
    {
        encoder_base_class_deinit(&sbc_enc_vt);
        free_data_and_fail(op_data, response_data);
        return TRUE;
    }
...
{
    SBC_ENC_OP_DATA *sbc_data = get_instance_data(op_data);
    sbc_encode_lib_reset(sbc_data->encoder_data.codec.encoder_data_object);
...
{
    SBC_ENC_OP_DATA *op_extra_data = get_instance_data(op_data);
