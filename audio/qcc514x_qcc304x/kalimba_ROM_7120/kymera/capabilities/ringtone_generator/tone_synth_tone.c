...
int16 tone_synth_sin_8bit(unsigned x)
{
...
bool tone_synth_tone_play(TONE_SYNTH_DATA *synth_data, unsigned num_of_samples)
{
    ...
    {
        v = (unsigned) (synth_data->tone_data.volume >> 16);
        ...
        case tone_synth_tone_triangle2:
            ...
            samp = (int16)((((((unsigned) samp) >> 8) * 343) >> 8) - 0x80);
            break;
        ...
        case tone_synth_tone_plucked:
            s = (uint16) samp >> 8;
            samp = (int16)(((0x7f - s)*v + tone_synth_sin_8bit(s)*(256-v))>>8);
            break;
        }
