...
typedef struct WBS_ENC_OP_DATA
{
    ...
    /** Kick forward each time WBS_ENC is kicked (to kick SCO_SINK_EP on TESCO rate) */
...
typedef struct WBS_DEC_OP_DATA
{
    ...
    unsigned wbs_dec_invalid_validate_result;  /*  WBS validate returned an invalid result */
