...
/**
 * \brief Get the sampling frequency of audio in this sco connection
 * \param type Either SCO_NB, WBS, UWBS or SWBS
 * \returns Sampling Frequency; 0 if type was wrong
 */
static inline unsigned get_sampling_freq(CONNECTION_TYPE type)
{
    switch (type)
    {
        case SCO_NB:
...
/**
 * \brief Translates the metadata header from SCO to AudioSS endiannes
 * \param data Pointer to buffered metadata
 * \param type Either SCO_NB, WBS, UWBS or SWBS
 * \param sco_metadata Translated, readable metadata header
 */
 ...
 * \param type - Type of the packet. 0 NB, 1 WB, 2 UWB, 3 SWB.
...
stream_sco_metadata_status analyse_sco_metadata(SCO_COMMON_RCV_OP_DATA* sco_data, stream_sco_metadata *sco_metadata)
{
    ...
    if ( time_diff & 0x8000 )
    {
        ...
        {
            L3_DBG_MSG3("Warning! Packet time %u < Expected time %u - t_esco %u /2. Packet late. ",
                                                time_stamp,
                                                sco_data->sco_rcv_parameters.expected_time_stamp,
    ...
    else
    {
        ...
        {
            L3_DBG_MSG3("Warning! Packet time %u > Expected time %u + t_esco %u /2. Packet early. ",
                                                time_stamp,
                                                sco_data->sco_rcv_parameters.expected_time_stamp,
                                                sco_data->sco_rcv_parameters.t_esco);
 ...
 * \param type - connection type (SCO_NB, WBS, UWBS or SWBS) only used to determine sample rate
 */
 ...
 * \param sco_data - Pointer to the common SCO rcv operator data
 * \param type - connection type (SCO_NB, WBS, UWBS or SWBS)
 */
 ...
 * \param sco_data Pointer to the common SCO rcv data
 * \param sco_metadata Pointer to the sco_metadata copy
 */
void sco_rcv_update_status_counters(SCO_COMMON_RCV_OP_DATA* sco_data,
                                    stream_sco_metadata* sco_metadata)
{
    switch (sco_metadata->status)
    {
        case OK:
        {
            sco_data->sco_rcv_parameters.md_status_ok++;
            break;
        }
        case CRC_ERROR:
        case CRC_ERROR_WITH_BIT_ERRORS:
        {
            sco_data->sco_rcv_parameters.md_status_crc++;
            L3_DBG_MSG1("Packet status CRC_ERROR, cnt= %x",
                         sco_data->sco_rcv_parameters.md_status_crc);
            break;
        }
        case NOTHING_RECEIVED:
        {
            sco_data->sco_rcv_parameters.md_status_nothing ++;
            L3_DBG_MSG1("Packet status NOTHING_RECEIVED, cnt= %x",
                         sco_data->sco_rcv_parameters.md_status_nothing);
            break;
        }
        case NEVER_SCHEDULED:
        {
            sco_data->sco_rcv_parameters.md_status_not_sched ++;
            L3_DBG_MSG1("Packet status NEVER_SCHEDULED, cnt= %x",
                         sco_data->sco_rcv_parameters.md_status_not_sched);
            break;
        }
    }
}
#endif /* SCO_DEBUG_STATUS_COUNTERS */
