...
static void groan_over_kip(TIME t, const FAULTINFO *fi, const uint16 *args16)
{
    /**
     * Talking to Curator is restricted to P0. Send a request to P0 to publish
     * the fault.
     */
    PROC_ID_NUM local_proc;
    CONNECTION_LINK con_id;
    ADAPTOR_MSGID msg_id;
    KIP_MSG_PUBLISH_FAULT_REQ* req_msg;

    local_proc = proc_get_processor_id();
    con_id = PACK_CON_ID(PACK_SEND_RECV_ID(PROC_PROCESSOR_0,0),
                         PACK_SEND_RECV_ID(local_proc,0));

    req_msg = xpmalloc(sizeof(KIP_MSG_PUBLISH_FAULT_REQ) +
    ...
    KIP_MSG_PUBLISH_FAULT_REQ_FAULT_ID_SET(req_msg, fi->f);
    KIP_MSG_PUBLISH_FAULT_REQ_PROCESSOR_SET(req_msg, local_proc);
    KIP_MSG_PUBLISH_FAULT_REQ_NUM_REPORTS_SET(req_msg, fi->n);
    KIP_MSG_PUBLISH_FAULT_REQ_TIMESTAMP_SET(req_msg, t);
    KIP_MSG_PUBLISH_FAULT_REQ_ARGS_LEN_SET(req_msg, ARGS_ARRAY_LENGTH);
