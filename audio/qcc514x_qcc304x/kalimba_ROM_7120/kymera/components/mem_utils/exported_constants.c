 ...
 */
static void send_exp_const_kip_msg(PROC_ID_NUM proc,
                                   exp_const_msgid id,
                                   void *const_table,
                                   unsigned operator,
                                   void *callback)
{
    KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ kip_msg;
    uint16 con_id;
    ADAPTOR_MSGID msg_id;
    con_id = (uint16) PACK_CON_ID(PACK_SEND_RECV_ID(proc_get_processor_id(),0),
                                  PACK_SEND_RECV_ID(proc,0));

    KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_MI_SET(&kip_msg, id);
    KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_CONSTANT_P_SET(&kip_msg,
                                                         (uintptr_t) const_table);
    KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_OPERATOR_ID_SET(&kip_msg, operator);
    KIP_MSG_EXPORTED_CONSTANTS_ACTION_REQ_CALLBACK_INFO_SET(&kip_msg,
                                                            (uintptr_t) callback);
 ...
 */
static void send_callback_over_kip(PROC_ID_NUM proc, void *callback_struct)
{
    send_exp_const_kip_msg(proc,EXP_CONST_MSG_CALLBACK_ID,NULL,(unsigned)-1,callback_struct);
}
#else
#define send_exp_const_kip_msg(x1, x2, x3, x4, x5)
#define send_callback_over_kip(x, y)
#endif /* defined(SUPPORTS_MULTI_CORE) */
...
static void send_message_to_task(exp_const_msgid id, void *const_table, unsigned operator, exp_const_msg_cb *cb_param)
{
    ...
    {
        send_exp_const_kip_msg(PROC_PROCESSOR_0,id,const_table,operator,cb_param);
        return;
    }
    
    msg = zpnew(exp_const_msg);
    msg->const_table = const_table;
    msg->operator = operator;
    msg->callback = cb_param;
    msg->proc = proc_get_processor_id();
...
static void process_activity_queue(void)
{
    ...
    case EXP_CONST_MSG_CALLBACK_ID:
        ...
        if (!PROC_ON_SAME_CORE(current->proc))
        {
...
#endif /* defined(INSTALL_CAPABILITY_CONSTANT_EXPORT) */
