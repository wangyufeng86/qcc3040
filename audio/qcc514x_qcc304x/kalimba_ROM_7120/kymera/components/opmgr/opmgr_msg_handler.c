                                                              ...
                                                              STATUS_KYMERA status)
{
    ...
    con_id = REVERSE_CONNECTION_ID((CONNECTION_LINK) rinfo->dest_id);
#else
    con_id = PACK_CON_ID((CONNECTION_PEER) rinfo->src_id,
                         (CONNECTION_PEER) rinfo->dest_id);
#endif
    callback(con_id, status, data->cur_index, msg->resp_data.err_code);
...
static void msg_from_op_handler(tRoutingInfo *rinfo, uint16f *msg)
{
    ...
    adaptor_send_message(con_id, AMSGID_FROM_OPERATOR,
...
static void message_resp_handler(tRoutingInfo *rinfo, OP_OPMSG_RSP *msg)
{
    ...
    rinfo->src_id = GET_SRC_ID_FROM_PACKED_SRC_ID((CONNECTION_LINK)rinfo->src_id);
    con_id = PACK_CON_ID((CONNECTION_PEER) rinfo->src_id,
                         (CONNECTION_PEER) rinfo->dest_id);

    callback = (OP_MSG_CBACK) opmgr_retrieve_in_progress_task(rinfo->dest_id,
                                                              op_id);
    if (callback == NULL)
    {
        /* No data, panic. Passing operator id as debug information. */
    ...
    if(msg != NULL)
    {
        ...
        callback(con_id, (STATUS_KYMERA) msg->status, INT_TO_EXT_OPID(msg->op_id), msg->length, (void*)&(msg->payload));
    }
    else
    {
...
static void create_resp_handler(tRoutingInfo *rinfo, OP_STD_RSP *msg)
{
    ...
    callback = (OP_CREATE_CBACK) opmgr_retrieve_in_progress_task(rinfo->dest_id, op_id);
    ...
    if (STATUS_OK == msg->status)
    {
        ...
        callback(con_id, (STATUS_KYMERA) msg->status, INT_TO_EXT_OPID(op_id));
