...
#define USB_AUDIO_TX_HOST_PAUSE_THRESHOLD_PACKETS            20  /* No activity threshold for deciding host
                                                                  * has paused (in number of packets)
                                                                  */
#define USB_AUDIO_TX_HOST_PAUSE_THRESHOLD_KICKS    \
    (USB_AUDIO_TX_HOST_PAUSE_THRESHOLD_PACKETS/USB_AUDIO_TX_PACKETS_PER_KICK) /* in kicks */
/**
...
/**
 * stream_usb_audio_silence_sink_buffer
 * \brief silences already written usb audio packets
 * \param sink_buffer pointer to cbuffer for destination buffer, this is normally
 *        a local mmu buffer
 * \param silence_in_octets total size of packets in octets
 */
extern void stream_usb_audio_silence_sink_buffer(tCbuffer *sink_buffer, unsigned silence_in_octets);
