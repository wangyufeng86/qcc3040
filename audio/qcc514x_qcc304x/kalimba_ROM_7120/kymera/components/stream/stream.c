 ...
 *
 */
CONNECTION_LINK stream_con_id_from_ep(ENDPOINT *ep)
{
    if (ep == NULL)
    {
        return INVALID_CON_ID;
    }
    return (CONNECTION_LINK) ep->con_id;
}
                                             ...
                                             uint32 *result_value)
{
    ...
    success = endpoint->connected_to->functions->get_config(endpoint->connected_to,
                                                            (unsigned) key,
                                                            &result);
 ...
 *
 */
void stream_close_endpoints_with_con_id(CONNECTION_LINK con_id)
{
    ENDPOINT *ep;
    ENDPOINT *tmp;
    unsigned int idx;
...
void stream_set_endpoint_connection_id(ENDPOINT **ep_list,
                                       unsigned num_eps,
                                       CONNECTION_LINK con_id)
{
    unsigned i;
