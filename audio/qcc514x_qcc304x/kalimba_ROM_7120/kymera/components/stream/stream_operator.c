...
static bool operator_close (ENDPOINT *endpoint);
 ...
 *
 */
ENDPOINT *stream_create_operator_endpoint(unsigned opidep,
                                          CONNECTION_LINK con_id)
{
    ENDPOINT *ep;
...
static bool operator_configure(ENDPOINT *endpoint, unsigned int key, uint32 value)
{
    ...
    case EP_CBOPS_PARAMETERS:
    {
        ...
        /* For EP_CBOPS_PARAMETERS, the only value we have at the moment is the shift */
        int shift;
        shift = get_shift_from_cbops_parameters((CBOPS_PARAMETERS *)(uintptr_t)value);
        if (opmgr_config_msg_to_operator(stream_external_id_from_endpoint(endpoint), \
                transfom_endpoint_key_to_operator_key(key), shift))
        {
            retval = TRUE;
        }
...
static OPMSG_CONFIGURATION_KEYS transfom_endpoint_key_to_operator_key(unsigned int key)
{
    ...
    {
        case EP_CBOPS_PARAMETERS:
            return OPMSG_OP_TERMINAL_SHIFT;
        case EP_DATA_FORMAT:
