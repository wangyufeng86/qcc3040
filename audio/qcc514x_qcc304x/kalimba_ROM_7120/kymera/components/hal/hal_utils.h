...
/**
 * \brief  Set a single field in a uint16 variable as if it were the given register, so
 * that the variable can later be used to set the register itself.
 *
 * \param  var  uint16 variable to be set.
 *
 * \param  register  register which is to be compared
 *
 * \param  field  field to be set.
 ...
 * \par
 * EXAMPLE
 *       hal_set_uint16_var_for_register_field(tardiness, CLKGEN_MINMAX_MMU_RATE,
 *            CLKGEN_MIN_MMU_RATE, min_rate);
 *
 */
#define hal_set_uint16_var_for_register_field(var, register, field, value) \
    CHECK_FIELD_TYPE(register, field)               \
    ((void) ((var) = (uint16)(((var) & ~FIELD_MASK_SHIFTED(field)) | \
     ((value) << FIELD_SHIFT(field)))))
