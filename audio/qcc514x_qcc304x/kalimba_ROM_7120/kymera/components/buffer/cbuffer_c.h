...
#define BUF_DESC_CBOPS_IS_SCRATCH(x)        ((x) & BUF_DESC_CBOPS_IS_SCRATCH_MASK)
   ...
   /* bit 15:      Obsolete metadata flag; use buff_has_metadata(cbuff).
    */
...
/**
 * Sets the usable octets per word.
 *
...
/**
 *  Calculate amount of samples read(consumed)/written(arrived) from/into
 *  the buffer since last call.
 *
 * \param cbuffer pointer to cbuffer to calculate the new amount
 * \param last_addr pointer to a variable used for storing last address
 * \param is_output if TRUE will calculate amount read else amount written
 * \return new amount that has read/written from/into the buffer since
 *         previous call (note: first call will populate(initialise) last_addr
 *         but the return value will be invalid)
 */
extern unsigned cbuffer_calc_new_amount(tCbuffer *cbuffer, unsigned *last_addr, bool is_output);
...
/*
 * \brief Pack a 32-bit aligned buffer to a 16-bit aligned buffer
 *
 * \param dst destination cbuffer pointer
 * \param src source cbuffer pointer
 * \param num_words number of words to pack
 */
extern void cbuffer_copy_audio_to_packed(tCbuffer * dst, tCbuffer *src, unsigned num_words);

/*
 * \brief Unpack a 16-bit aligned buffer to a 32-bit aligned buffer
 *
 * \param dst destination cbuffer pointer
 * \param src source cbuffer pointer
 * \param num_words number of words we want to unpack (refers to dst buffer)
 */
extern void cbuffer_copy_packed_to_audio(tCbuffer * dst, tCbuffer *src, unsigned num_words);
