...
.MODULE $M.aacdec.huffman_reset_unpacked_list;
   ...
   r10 = 12;
   r1 = $aacdec.HUFFMAN_TABLE_USE_FROM_ROM;
   r0 = $aacdec.HUFFMAN_TABLE_NOT_UNPACKED;
   Null = M[r9 + $aac.mem.use_huffman_tables_from_rom];
   if NZ r0 = r1;
...
.MODULE $M.aacdec.huffman_unpack_individual_flash_table;
   ...
   Words2Addr(r5);
   r1 = r9 + r5;
   r0 = M[$aac.mem.huffman_cb_table + r1];
   Null = r0 - $aacdec.HUFFMAN_TABLE_NOT_UNPACKED;
   if EQ jump unpack_table;
   Null = r0 - $aacdec.HUFFMAN_TABLE_USE_FROM_ROM;
   if NZ rts;
      r0 = M[$aac.mem.huffman_packed_list + r1];
      M[$aac.mem.huffman_cb_table + r1] = r0;
      rts;
...
unpack_table:
   ...
   I3 = r1;
#ifdef K32
   // The huffman codebooks are already unpacked;
   // we need to copy them to the buffer
   //    addr 0:  bits 31-24 (zero padding), bits 23-0 (codebook word 0)
   //    addr 4:  bits 31-24 (zero padding), bits 23-0 (codebook word 1)
   //    ....
   //    etc
   r10 = r10 LSHIFT 1;
   do flash_copy_loop;
