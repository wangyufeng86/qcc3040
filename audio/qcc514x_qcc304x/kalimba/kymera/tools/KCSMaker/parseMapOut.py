    ...
    for mod in mod_list[1:]:
        if mod.get_low_pc() != prev_mod.get_high_pc() + 1:
            if mod.get_low_pc() > prev_mod.get_high_pc() + 1 + MAX_GAP_MINIM_BYTES:
                ...
                logging.debug("Gap between {m1} and {m2} is {m3} bytes".format(m1=prev_mod,m2=mod,m3=str(mod.get_low_pc() - prev_mod.get_high_pc())))
            if firstGap:
                firstGap = False
            else:
                logging.debug("More than 1 gaps in PM sections. Possibly caused by #pragma optimlevel 3?")
        prev_mod = mod
    f.close()
    return mod_list
