...
struct SECURE_BASIC_PASSTHROUGH_LICENSING
{
    unsigned         state;
 ...
 * assuming a device pskey_bdaddr={0x00af01,0x5b,0x0002} in 'subsys1_config2.htf'.
 */
bool basic_passthrough_retrieve_license_req(OPERATOR_DATA *op_data, CAP_ID capid)
{
    unsigned id = (unsigned) capid;
    PS_KEY_TYPE key;

    L2_DBG_MSG("[1] Request license key");

    lic = xzpnew(SECURE_BASIC_PASSTHROUGH_LICENSING);
    if (NULL == lic)
    {
        return FALSE;
    }
     ...
     *required for a customer, the ucid can be used for something else.
     */
    key = MAP_CAPID_UCID_SBID_TO_PSKEYID(id&(~0x3F), id&0x3F, OPMSG_P_STORE_PARAMETER_SUB_ID);
    ps_entry_read(op_data, key, PERSIST_ANY, secure_basic_passthrough_retrieve_license_callback);
