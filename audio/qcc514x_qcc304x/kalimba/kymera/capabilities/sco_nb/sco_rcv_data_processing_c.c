...
unsigned sco_rcv_processing(SCO_COMMON_RCV_OP_DATA *sco_data)
{
    ...
    if (!enough_data_to_run(sco_data, METADATA_HEADER_SIZE))
    {
         ...
         */
        L2_DBG_MSG3("t=%08x: Kick: %d, bad kicks: %d:: Warning! Not enough input data to run, faking packet",
            time_get_time(), sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.num_bad_kicks);
        ...
        switch(status)
        {
            case SCO_METADATA_LATE:
            {
                    ...
                    !enough_space_to_run(sco_data, sco_rcv_get_output_size_words(sco_data)))
                {
                     ...
                     * we will have enough data on the output.
                     */
                    L2_DBG_MSG3("t=%08x: Kick: %d, Late: %d:: Warning! Packet late and not enough input data or not enough output data to run again",
...
static unsigned copy_packet(SCO_COMMON_RCV_OP_DATA* sco_data, stream_sco_metadata* sco_metadata)
{
...
#else
    /* TODO get_packet_payload_length looks at in-band metadata packet_length;
     * this is a property of the input buffer and shouldn't make assumptions
     * about the output; probably that's why this #ifdef was added;
     * but then sco_rcv_get_output_size_words bakes-in the CVSD conversion ratio;
     * similar comments for below calculation of input_amount that >>1
     */
    ...
    output_words = (space_limited ? out_space : output_words);
    amount_copied = output_words;
    if (sco_metadata->status == OK)
    {
        cvsd_receive_asm(&sco_data->sco_rcv_parameters.cvsd_struct, sco_data->buffers.ip_buffer,
                sco_data->buffers.op_buffer, sco_data->sco_rcv_parameters.ptScratch, output_words);
    }
    else
    {
        /* will fake a packet (PLC) if enough space, discard all payload, if any */
        cbuffer_discard_data(sco_data->buffers.ip_buffer, get_packet_payload_length(sco_metadata));
                
#ifdef INSTALL_PLC100
        sco_data->sco_rcv_parameters.md_pkt_faked++;
        L3_DBG_MSG3("SCO_METADATA_ON_TIME but status %d (not OK),\
                 md_num_kicks= %d, md_pkt_faked= %d",
                 sco_metadata->status,
                 sco_data->sco_rcv_parameters.md_num_kicks,
                 sco_data->sco_rcv_parameters.md_pkt_faked);
#endif
    ...
    /* both input and output buffers metadata need updating */
    ...
    if (amount_copied < output_words)
    {
         ...
         * up here. */
        L2_DBG_MSG3("Kick nr %d. OOPS! amount_copied %d < get_packet_payload_length(current_packet) %d.",
                                            sco_data->sco_rcv_parameters.md_num_kicks,
                                            amount_copied,
                                            output_words);
#if 0
