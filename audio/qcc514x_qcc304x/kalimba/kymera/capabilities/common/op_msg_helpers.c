...
#ifdef USE_ACCMD
static bool client_is_obpm(const OPMSG_HEADER *opmsg_header)
{
    unsigned client_id = OPMGR_GET_OPCMD_MESSAGE_CLIENT_ID(opmsg_header);
    return opmgr_get_adaptor((CONNECTION_PEER) client_id) == OPMGR_ADAPTOR_OBPM;
}
#else
#define client_is_obpm(x) TRUE
#endif
...
bool cpsGetParameterMsgHandler(CPS_PARAM_DEF *param_def_ptr,void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
    ...
    if (client_is_obpm((OPMSG_HEADER*) message_data))
    {
...
void cps_response_set_result(OP_OPMSG_RSP_PAYLOAD **resp_data,
...
bool common_obpm_response_helper(OP_MSG_REQ *op_msg, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data, unsigned size)
{
    ...
    OPMSG_RESULT_STATES status = OPMSG_RESULT_STATES_NORMAL_STATE;
    ...
    *resp_data = resp;
    ...
    resp->u.cps.status = (unsigned) status;
