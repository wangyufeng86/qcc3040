    ...
    if (tag != NULL)
    {
        ...
        else if (IS_TIMESTAMPED_TAG(tag) || IS_TIME_OF_ARRIVAL_TAG(tag))
        {
            ...
            else
            {
                ...
                gap_to_start_of_buffer_w = gap_samples - (int)rcv_beforeidx_samples;
                ...
                if (gap_to_start_of_buffer_w < 0)
                {
                    /* New data appears to overlap old data */
                    return SRC_SYNC_STALL_RECOVERY_RESTART;
                }
                else
                {
                     ...
                     */
                    *p_gap_samples = (unsigned)gap_to_start_of_buffer_w;
