 ...
 *  NB. The legacy encoder frame_processing functions (asm implementation)
 *  are not C standard;
 *  the typical <codec>_frame_encode function would take as argument
 *  r5 = pointer to a $codec.ENCODER_STRUC structure
 *  (for details see the magic in the encoder_encode wrapper in encoder_cap.asm )
 */
typedef void (*ENCODE_FN)(void);

/** Function prototype of function that gets frame sizes
 * NB. return TRUE for frame-based encoder, FALSE for sample-based.
 */
...
typedef struct
{
    /** An ENCODER structure used by the codec library */
    ENCODER codec;
