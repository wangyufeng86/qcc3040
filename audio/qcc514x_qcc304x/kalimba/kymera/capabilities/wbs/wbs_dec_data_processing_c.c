...
unsigned wbs_dec_processing(OPERATOR_DATA *op_data)
{
    ...
    if (!enough_data_to_run(sco_data, METADATA_HEADER_SIZE))
    {
         ...
         */
        L2_DBG_MSG3("t=%08x: Kick: %d, bad kicks: %d:: Warning! Not enough data to run for the first time", time_get_time(), sco_data->sco_rcv_parameters.md_num_kicks, sco_data->sco_rcv_parameters.num_bad_kicks);
...
static unsigned decode_packet(OPERATOR_DATA *op_data, stream_sco_metadata* sco_metadata)
{
    ...
    else
    {
        ...
        else if (enough_space_to_run(sco_data, OUTPUT_BLOCK_SIZE))
        {
        ...
        {
		    /* On-time input but the output buffer is full. */
            SCO_DBG_MSG("Warning, not enough space to produce output");
            return TOUCHED_NOTHING;
        }
