...
MAP_INSTANCE_DATA(CAP_ID_CHANNEL_MIXER, GEN_MIXER_OP_DATA)
...
static void mixer_stall_timer_handler(void* kick_object)
{
    ...
    GEN_MIXER_OP_DATA *op_extra_data = get_instance_data(op_data);
        ...
        void *message_data, unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD  **response_data)
{
   ...
   num_sinks   = OPMSG_FIELD_GET(message_data, OPMSG_CH_MIXER_SET_PARAMETERS, NUM_INPUT_CHANNELS);
   num_sources = OPMSG_FIELD_GET(message_data, OPMSG_CH_MIXER_SET_PARAMETERS, NUM_OUTPUT_CHANNELS);
   ...
   no_of_gain_parameters = OPMGR_GET_OPMSG_LENGTH((OP_MSG_REQ *)message_data) - OPMSG_CH_MIXER_SET_PARAMETERS_GAIN_TABLE_WORD_OFFSET;
   ...
   gain_table = (int *) OPMSG_FIELD_POINTER_GET(message_data, OPMSG_CH_MIXER_SET_PARAMETERS, GAIN_TABLE);
   for(i = 0; i < num_sources; i++)
   {
       GEN_MIXER_SOURCE_INFO *src_ptr;
       unsigned               j;
                           ...
                           unsigned *resp_length, OP_OPMSG_RSP_PAYLOAD **resp_data)
{
   ...
   mixer_data->sample_rate = 25 * (OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_SAMPLE_RATE, SAMPLE_RATE));
...
#ifdef MIXER_SUPPORTS_STALLS
    /* Kill timer Task */
    timer_cancel_event_atomic(&mixer_data->kick_id);
#endif
...
void setup_mixers(GEN_MIXER_OP_DATA *mixer_data)
{
    ...
    mixer_data->time_stamp      = (unsigned)time_get_time();
