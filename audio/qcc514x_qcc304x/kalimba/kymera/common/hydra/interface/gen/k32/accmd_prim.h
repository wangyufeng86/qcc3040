 ...
 VALUES
    ...
    STREAM_CODEC_OUTPUT_QUALITY
                   - Configure an output quality vs power consumption tradeoff.
                     ...
                     quality.
    STREAM_CODEC_AOV_MODE_ON
                   - When set, enables AOV-continuous buffering mode on analog
                     mic interface.
    STREAM_CODEC_AOV_MAX_ENTRIES
                   - Configure the threshold of aov buffer. After this threshold
                     is reached, an interrupt is raised by aov hardware.
    ...
    STREAM_DIGITAL_MIC_AOV_MODE_ON
                   - When set, enables AOV-continuous buffering mode on digital
                     mic interface.
    STREAM_DIGITAL_MIC_AOV_MAX_ENTRIES
                   - Configure the threshold of aov buffer. After this threshold
                     is reached, an interrupt is raised by aov hardware.
...
typedef enum
{
    ...
    ACCMD_CONFIG_KEY_STREAM_CODEC_AOV_MODE_ON = 0x031b,
    ACCMD_CONFIG_KEY_STREAM_CODEC_AOV_MAX_ENTRIES = 0x031c,
    ...
    ACCMD_CONFIG_KEY_STREAM_DIGITAL_MIC_AOV_MODE_ON = 0x060d,
    ACCMD_CONFIG_KEY_STREAM_DIGITAL_MIC_AOV_MAX_ENTRIES = 0x060e,
...
typedef enum
{
    ACCMD_DDS_CLK_32MHZ = 32,
    ACCMD_DDS_CLK_80MHZ = 80
} ACCMD_DDS_CLK;
/*******************************************************************************
...
typedef enum
{
    ...
    ACCMD_SIGNAL_ID_MAP_CPU_SPEED_REQ = 0x0044,
    ACCMD_SIGNAL_ID_DDS_CLOCK_CONFIGURE_REQ = 0x0045,
...
#define ACCMD_UNSOLICITED_SYSTEM_FRAMEWORK_REQ_KEY_GET(addr) ((SYSTEM_KEYS_UNSOL_MSG)*((addr) + 3))
...
/*******************************************************************************
  ...
  NAME
    Accmd_Map_CPU_Speed_Req

  DESCRIPTION
    From Client Map an abstract CPU speed to a concrete clock frequency. This
    commands is not available on all chips. Response is a Standard_Resp.

  MEMBERS
    speed     - Abstract CPU speed to change. Do not use NO_CHANGE here.
    frequency - Actual CPU clock frequency to be used. Usable frequency range
                and accuracy is dependent on the chip.
...
/* The following macros take ACCMD_MAP_CPU_SPEED_REQ *accmd_map_cpu_speed_req_ptr or uint16 *addr */
#define ACCMD_MAP_CPU_SPEED_REQ_SPEED_WORD_OFFSET (3)
#define ACCMD_MAP_CPU_SPEED_REQ_SPEED_GET(addr) ((ACCMD_CPU_CLK)*((addr) + 3))
#define ACCMD_MAP_CPU_SPEED_REQ_SPEED_SET(addr, speed) (*((addr) + 3) = (uint16)(speed))
#define ACCMD_MAP_CPU_SPEED_REQ_FREQUENCY_WORD_OFFSET (4)
#define ACCMD_MAP_CPU_SPEED_REQ_FREQUENCY_GET(addr) (*((addr) + 4))
#define ACCMD_MAP_CPU_SPEED_REQ_FREQUENCY_SET(addr, frequency) (*((addr) + 4) = (uint16)(frequency))
#define ACCMD_MAP_CPU_SPEED_REQ_WORD_SIZE (5)
#define ACCMD_MAP_CPU_SPEED_REQ_PACK(addr, speed, frequency) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(speed)); \
        *((addr) + 4) = (uint16)((uint16)(frequency)); \
    } while (0)

#define ACCMD_MAP_CPU_SPEED_REQ_MARSHALL(addr, accmd_map_cpu_speed_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_map_cpu_speed_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_map_cpu_speed_req_ptr)->speed); \
        *((addr) + 4) = (uint16)((accmd_map_cpu_speed_req_ptr)->frequency); \
    } while (0)

#define ACCMD_MAP_CPU_SPEED_REQ_UNMARSHALL(addr, accmd_map_cpu_speed_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_map_cpu_speed_req_ptr)->header)); \
        (accmd_map_cpu_speed_req_ptr)->speed = ACCMD_MAP_CPU_SPEED_REQ_SPEED_GET(addr); \
        (accmd_map_cpu_speed_req_ptr)->frequency = ACCMD_MAP_CPU_SPEED_REQ_FREQUENCY_GET(addr); \
    } while (0)
...
/*******************************************************************************
  ...
  NAME
    Accmd_Dds_Clock_Configure_Req

  DESCRIPTION
    From Client This command allow the APP to register desired DDS clock used
    for the I2S/PCM/SPDIF (At the moment 32/80MHz). This commands is not
    available on all chips. Response is a Standard_Resp.

  MEMBERS
    dds_clk_freq - Clock Configuration for DDS.

*******************************************************************************/
typedef struct
{
    ACCMD_HEADER  header;
    ACCMD_DDS_CLK dds_clk_freq;
} ACCMD_DDS_CLOCK_CONFIGURE_REQ;

/* The following macros take ACCMD_DDS_CLOCK_CONFIGURE_REQ *accmd_dds_clock_configure_req_ptr or uint16 *addr */
#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_DDS_CLK_FREQ_WORD_OFFSET (3)
#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_DDS_CLK_FREQ_GET(addr) ((ACCMD_DDS_CLK)*((addr) + 3))
#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_DDS_CLK_FREQ_SET(addr, dds_clk_freq) (*((addr) + 3) = (uint16)(dds_clk_freq))
#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_WORD_SIZE (4)
#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_PACK(addr, dds_clk_freq) \
    do { \
        *((addr) + 3) = (uint16)((uint16)(dds_clk_freq)); \
    } while (0)

#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_MARSHALL(addr, accmd_dds_clock_configure_req_ptr) \
    do { \
        ACCMD_HEADER_MARSHALL((addr), &((accmd_dds_clock_configure_req_ptr)->header)); \
        *((addr) + 3) = (uint16)((accmd_dds_clock_configure_req_ptr)->dds_clk_freq); \
    } while (0)

#define ACCMD_DDS_CLOCK_CONFIGURE_REQ_UNMARSHALL(addr, accmd_dds_clock_configure_req_ptr) \
    do { \
        ACCMD_HEADER_UNMARSHALL((addr), &((accmd_dds_clock_configure_req_ptr)->header)); \
        (accmd_dds_clock_configure_req_ptr)->dds_clk_freq = ACCMD_DDS_CLOCK_CONFIGURE_REQ_DDS_CLK_FREQ_GET(addr); \
    } while (0)
