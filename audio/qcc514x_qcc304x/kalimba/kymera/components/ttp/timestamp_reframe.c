...
void reframe_check_tags(REFRAME *reframe)
{
    ...
    while(tag)
    {
        ...
        if (reframe->tag.is_void != is_void_tag)
        {
            ...
            bool before_tag = ((reframe->prev_tag.length - reframe->prev_tag.octets_consumed < buff_size) &&
    ...
    if ((last_timestamped_tag != NULL)&&(!reframe->tag.is_void))
    {
        ...
        /* Check whether the new tag is within the current reframe interval or not.
         * reframe->prev_tag.length should not be zero because only tags before 
         * are checked during the start.  
         */
        if (reframe->prev_tag.length >= octets_to_tag)
        {
            /* New tag is within the reframing interval.
             * Extrapolate forwards to calculate the new timestamp.
             */
        ...
        else
        {
            ...
            /* New tag is beyond the reframing interval.
             * Work backwards to calculate the new timestamp.
             */
            octets_to_tag = octets_to_tag - reframe->prev_tag.length;
            samples_to_tag = octets_to_tag / OCTETS_PER_SAMPLE;

            TTP_DBG_MSG3("reframe: samples_to_tag = 0x%08x, diff = 0x%08x, correct = 0x%08x ",
                    samples_to_tag, reframe->tag.index - last_timestamped_tag->index, samples_to_tag*OCTETS_PER_SAMPLE);

            reframe->tag.timestamp = ttp_get_prev_timestamp(last_timestamped_tag->timestamp, samples_to_tag, reframe->sample_rate, last_timestamped_tag->sp_adjust);
        }
    }
