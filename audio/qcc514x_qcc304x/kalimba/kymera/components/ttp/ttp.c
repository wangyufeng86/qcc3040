...
struct ttp_context
{
    ...
    int ttp_frac;
    /** Following fields are used for TWM use cases */
    bool startup_ttp_override;
    ttp_mode mode;
 ...
 * initilaise_ttp
 *
 * \brief Initialise the ttp and set the next state
 *
 */
static void ttp_startup(ttp_context *context, TIME time)
{
    /* First call with this context, so just set the initial TTP */
    if(context->startup_ttp_override == TRUE)
    {
        context->startup_ttp_override = FALSE;
        if(time_gt(time, context->ttp))
        {
            TTP_WARN_MSG2("ToA = %4d > ttp = %4d", time, context->ttp);
        }
    }
    else
    {
    ...
    if(context->mode == TTP_MODE_FREE_RUN || context->mode == TTP_MODE_FREE_RUN_ONLY)
    {
        context->state = TTP_STATE_FREE_RUNNING;
    }
    else
    {   
        context->state = TTP_STATE_STARTUP;
        context->countdown = context->params.startup_period;
        if ((context->countdown == DEFAULT_STARTUP_PERIOD) &&
...
/**
 * calculate_ttp_raw_error
 *
 * \brief calculate the raw error 
 *        latency error =   actual latency - target latency
 */
static TIME_INTERVAL calculate_ttp_raw_error(ttp_context *context, TIME time)
{
    TIME_INTERVAL raw_error, raw_latency;
...
/**
 * ttp_get_msg_sp_adjustment
 *
 * \brief  Extract sample period adjustment value provided in the operator message
 *
 */
int ttp_get_msg_sp_adjustment(void *message_data)
{
    patch_fn_shared(ttp_gen);

    PL_ASSERT(OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_SPADJ, MESSAGE_ID) == OPMSG_COMMON_SET_TTP_SPADJ);

    return get_msg_fractional( OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_SPADJ, SP_ADJ_MS), 
                               OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_SPADJ, SP_ADJ_LS));

}

/**
 * ttp_get_msg_adjust_ttp_timestamp
 *
 * \brief  Extract the signed ttp adjust value for modifying the next ttp timestamp
 *
 */
TIME_INTERVAL ttp_get_msg_adjust_ttp_timestamp(void *message_data)
{
    patch_fn_shared(ttp_gen);

    PL_ASSERT(OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP, MESSAGE_ID) == OPMSG_COMMON_ADJUST_TTP_TIMESTAMP);

    return get_msg_fractional( OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP, TTP_ADJ_MS), 
                               OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_ADJUST_TTP_TIMESTAMP, TTP_ADJ_LS));
}
...
/**
 * ttp_get_state_params_status
 *
 * \brief  get the current ttp state params status
 *
 */
void ttp_get_state_params_status(ttp_context *context, ttp_state_params *state_params,unsigned *ttp_resync_count) 
{
    patch_fn_shared(ttp_gen);

    PL_ASSERT(state_params != NULL);
    PL_ASSERT(ttp_resync_count != NULL);

    state_params->ttp = context->ttp;
    state_params->sp_adjustment = context->sp_adjustment;
    state_params->latency = context->target_latency;
    state_params->mode = context->mode;
    *ttp_resync_count =  context->resync_count;
}


/**
 * ttp_get_msg_state_params
 *
 * \brief  Extract TTP state parameters from operator message
 *
 */
void ttp_get_msg_state_params(ttp_state_params *state_params, void *message_data)
{
    patch_fn_shared(ttp_gen);

    PL_ASSERT(OPMSG_FIELD_GET( message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, MESSAGE_ID ) == OPMSG_COMMON_SET_TTP_STATE);

    /* decode the message data */
    state_params->ttp = (TIME)(OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, TTP_TIMESTAMP_LS)) + 
                        (TIME)(OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, TTP_TIMESTAMP_MS) << 16); 

    state_params->sp_adjustment = get_msg_fractional(
                                        OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, SP_ADJ_MS), 
                                        OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, SP_ADJ_LS));

    state_params->latency = (TIME_INTERVAL)(OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, LATENCY_LS)) + 
                            (TIME_INTERVAL)(OPMSG_FIELD_GET(message_data, OPMSG_COMMON_MSG_SET_TTP_STATE, LATENCY_MS) << 16); 

    state_params->mode = (ttp_mode)OPMSG_FIELD_GET(message_data,  OPMSG_COMMON_MSG_SET_TTP_STATE, MODE);
}
...
/**
 * ttp_configure_sp_adjustment
 *
 * \brief  Configure TTP sample period adjustment
 *
 */
bool ttp_configure_sp_adjustment(ttp_context *context, int sp_adjustment)
{
    patch_fn_shared(ttp_gen);

    /* Just copy the supplied value into the context structure if ttp is not running.
     * while running this value will be used for generating next time stamp. 
     */
    
    if (context->state != TTP_STATE_RUNNING)
    {
        TTP_WARN_MSG1("TTP sample period adjustment = %x", sp_adjustment);
        context->sp_adjustment = sp_adjustment;
        return TRUE;
    }
...
/**
 * ttp_adjust_ttp_timestamp
 *
 * \brief  Configure TTP sample period adjustment
 *
 */
void ttp_adjust_ttp_timestamp(ttp_context *context, TIME_INTERVAL ttp_adjust_ts)
{
    patch_fn_shared(ttp_gen);

    /* 
     * Updated the ttp which will be used to calculate next ttp
     * Adjust the previous source time as well which will be used to calculate the next ttp using source time.
     */
    context->ttp = time_add(context->ttp, ttp_adjust_ts);
    context->prev_source_time = time_sub(context->prev_source_time, ttp_adjust_ts);
    context->resync_count++;

    context->ttp_frac = 0;
    context->delta_remainder = 0;
    TTP_WARN_MSG3("TTP timestamp adjustment = %x, adjusted ttp =%4d , changed previous source time =%4d", ttp_adjust_ts, context->ttp, context->prev_source_time);
}
...
/**
 * ttp_configure_state_params
 *
 * \brief  Configure TTP state parameters
 */
void ttp_configure_state_params(ttp_context *context, const ttp_state_params *state_params)
{
    patch_fn_shared(ttp_gen);

    context->mode = state_params->mode;

    /* 
     * Update the parameter values only in TTP_MODE_FREE_RUN and TTP_MODE_FULL_TTP
     * Don't update the parameters otherwise.
     */
    if( context->mode == TTP_MODE_FREE_RUN || context->mode == TTP_MODE_FULL_TTP )
    {
        /* override the ttp with the new value */
        context->startup_ttp_override = TRUE;
        context->ttp = state_params->ttp;
        context->sp_adjustment = state_params->sp_adjustment;
        context->target_latency = state_params->latency;
        context->resync_count++;
        TTP_DBG_MSG5("TTP Generator 0x%08x: overridden ttp = %4d, sra = 0x%08x, latency = %4d mode =%d",
                         (uintptr_t)context,context->ttp, context->sp_adjustment, context->target_latency, context->mode);
    }
    else
    {
        TTP_DBG_MSG2("TTP Generator 0x%08x:switched mode =%d",(uintptr_t)context,context->mode);
    }

    if(context->state == TTP_STATE_FREE_RUNNING &&  
      (context->mode == TTP_MODE_FULL_TTP || context->mode == TTP_MODE_FULL_TTP_ONLY))
    {
        /* Immediately switch to RUNNING state */
        context->state = TTP_STATE_RUNNING;
    }
    else if(context->state == TTP_STATE_RUNNING &&       
           (context->mode == TTP_MODE_FREE_RUN || context->mode == TTP_MODE_FREE_RUN_ONLY))
    {
        context->state = TTP_STATE_FREE_RUNNING;
    }
    TTP_DBG_MSG2("TTP Generator 0x%08x: current state=%d",(uintptr_t)context,context->state);
}
...
void ttp_update_ttp_from_source_time(ttp_context *context, TIME toa, TIME source_time, ttp_status *status)
{
    ...
    if ((context->state != TTP_STATE_INIT) && (context->state != TTP_STATE_FREE_RUNNING))
    {
    ...
    if (context->state == TTP_STATE_INIT)
    {
        ...
        /* switch to startup state or free run mode */
        ttp_startup(context, toa);
    ...
    /* calculate the raw error only for TTP_STATE_RUNNING or TTP_STATE_STARTUP */  
    if ((context->state == TTP_STATE_STARTUP) || (context->state == TTP_STATE_RUNNING))
    {
        raw_error = calculate_ttp_raw_error(context, toa);
    }
    ...
    /* update the sp adjustment only while running. Don't update it in the free running mode*/
    if (context->state == TTP_STATE_RUNNING)
    {
...
void ttp_update_ttp(ttp_context *context, TIME time, unsigned samples, ttp_status *status)
{
    ...
    if (context->state == TTP_STATE_INIT)
    {
        ...
        /* change the state to startup */
        ttp_startup(context, time);
    }
    else
    {
        /* TTP for this block is based on the calculated duration for the previous one */
    ...
    /* calculate the raw error only  for states TTP_STATE_RUNNING and TTP_STATE_STARTUP */
    if((context->state == TTP_STATE_STARTUP) || (context->state == TTP_STATE_RUNNING))
    {
        raw_error = calculate_ttp_raw_error(context,time);
    }
...
/*
 * ttp_get_prev_timestamp
 *
 * \brief Function calculates the timestamp for a new tag based on a later tag.
 */
unsigned ttp_get_prev_timestamp(unsigned last_timestamp, unsigned nr_of_samples,
        unsigned sample_rate, int sp_adjust)
{
    unsigned advance = (unsigned)(((uint48)nr_of_samples * 1000000)/sample_rate);
    ...
    return time_sub(last_timestamp, advance);
}
