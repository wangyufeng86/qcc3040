...
static inline void tester_kick(ENDPOINT *ep, ENDPOINT_KICK_DIRECTION kick_dir)
{
    ...
    {
        if ( buff_has_metadata(tester->source_data_buf) )
        {
    ...
    if (copy_samples != 0)
    {
        ...
        if ((SOURCE == ep->direction && buff_has_metadata(tester->source_data_buf)) ||
            (SINK == ep->direction && buff_has_metadata(tester->sink_data_buf)) )
        {
        ...
        {
            if (buff_has_metadata(tester->sink_data_buf))
            {
