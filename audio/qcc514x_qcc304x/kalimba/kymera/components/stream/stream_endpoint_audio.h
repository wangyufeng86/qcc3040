...
/**
 * \brief Obtain the audio buffer length based on the minimum buffer size.
 *
 * \param buffer_size the minimum buffer size needed.
 *
 * \return length of the required buffer
 */
extern unsigned int stream_audio_get_hw_buff_size(unsigned buffer_size);
