...
#if defined(SUPPORTS_MULTI_CORE)
                PROC_ID_NUM proc_id;
                proc_id = opmgr_get_processor_id_from_opid(params[0]);
                if ((proc_id != PROC_PROCESSOR_INVALID) &&
                    (!PROC_ON_SAME_CORE(proc_id)))
                {
                    CONNECTION_LINK packed_con_id = PACK_CONID_PROCID(con_id, proc_id);
                    ...
                    stream_kip_operator_get_endpoint(params[0], params[1],
                                                     dir, packed_con_id,
                                                     callback);
                    return;
                }
                                        ...
                                        unsigned transform_id))
{
    ...
    PROC_ID_NUM remote_processor_id = PROC_PROCESSOR_0;
...
void stream_if_transform_from_ep(CONNECTION_LINK con_id,
                                 ...
                                 bool (*callback)(CONNECTION_LINK con_id,
                                                  ...
                                                  unsigned tr_id))
{
    CONNECTION_LINK dest_con_id = REVERSE_CONNECTION_ID(con_id);
    struct ENDPOINT *ep;
    TRANSFORM *tfm;
