...
typedef struct
{
     ...
     * Same as in cbops_shift_c.h, cbops_dc_remove_c.h, etc*/
    int shift_amount;
}CBOPS_SHIFT_DATA;
...
*/
        case CBOPS_DISCARD:
            return sizeof(CBOPS_DISCARD_DATA);
...
bool opmgr_override_get_hw_warp(OVERRIDE_EP_HANDLE ep_hdl, int* value)
{
    uint32 val = 0;
    bool result;

    result = stream_get_connected_to_endpoint_config((ENDPOINT *) ep_hdl,
                                                     OVERRIDE_CURRENT_HW_WARP,
                                                     0,
                                                     &val);
    *value = (int) val;
    return result;
...
bool opmgr_override_get_sample_rate(OVERRIDE_EP_HANDLE ep_hdl, unsigned *value)
{
    uint32 val = 0;
    bool result;

    result = stream_get_connected_to_endpoint_config((ENDPOINT *) ep_hdl,
                                                     OVERRIDE_AUDIO_SAMPLE_RATE,
                                                     0,
                                                     &val);
    *value = (unsigned) val;
    return result;
...
bool opmgr_override_set_ep_gain(OVERRIDE_EP_HANDLE ep_hdl, uint32 value)
{
    ENDPOINT *ep = (ENDPOINT*) ep_hdl;

    /* Set the gain for the overridden endpoint  */
    if (stream_direction_from_endpoint(ep) == SINK) /* connected to SOURCE */
    {
        return stream_configure_connected_to_endpoint(ep,
                                                      OVERRIDE_SET_INPUT_GAIN,
                                                      value);
    }
    else
    {
        return stream_configure_connected_to_endpoint(ep,
                                                      OVERRIDE_SET_OUTPUT_GAIN,
                                                      value);
    }
}

/* get_override_ep_ratematch_enacting */
bool opmgr_override_set_ratematch_enacting(OVERRIDE_EP_HANDLE ep_hdl,
                                           bool enable)
{
    return stream_configure_connected_to_endpoint((ENDPOINT *)ep_hdl,
                                                  OVERRIDE_RATEMATCH_ENACTING,
                                                  (uint32) enable);
}

/* get_override_ep_ratematch_ability */
bool opmgr_override_get_ratematch_ability(OVERRIDE_EP_HANDLE ep_hdl,
                                          RATEMATCHING_SUPPORT *value)
{
    return stream_get_connected_to_endpoint_config((ENDPOINT *) ep_hdl,
                                                   OVERRIDE_RATEMATCH_ABILITY,
                                                   RATEMATCHING_SUPPORT_NONE,
                                                   (uint32*) value);
}

#ifdef INSTALL_DELEGATE_RATE_ADJUST_SUPPORT
/* get_override_ep_rate_adjust_op */
bool opmgr_override_get_rate_adjust_op(OVERRIDE_EP_HANDLE ep_hdl,
                                       EXT_OP_ID *op_id)
{
    uint32 value = 0;
    bool result;
    ...
    result = stream_get_connected_to_endpoint_config((ENDPOINT *) ep_hdl,
                                                     OVERRIDE_RATE_ADJUST_OP,
                                                     0,
                                                     &value);
    *op_id = (EXT_OP_ID) value;
    return result;
}
...
{
    return stream_get_connected_to_endpoint_config((ENDPOINT *)ep_hdl,
                                                   OVERRIDE_RATEMATCH_RATE,
                                                   (1 << STREAM_RATEMATCHING_FIX_POINT_SHIFT),
                                                   value);
}

/* is_override_ep_locally_clocked */
bool opmgr_override_is_locally_clocked(OVERRIDE_EP_HANDLE ep_hdl)
{
    return stream_connected_to_endpoint_is_locally_clocked((ENDPOINT*)ep_hdl);
}

/* override_endpoints_have_same_clock_source */
bool opmgr_override_have_same_clock_source(OVERRIDE_EP_HANDLE ep_hdl1,
                                           OVERRIDE_EP_HANDLE ep_hdl2)
{
    return stream_connected_to_endpoints_have_same_clock_source((ENDPOINT*) ep_hdl1,
                                                                (ENDPOINT*) ep_hdl2);
}
/* opmgr_override_have_sidetone_route */
bool opmgr_override_have_sidetone_route(OVERRIDE_EP_HANDLE mic_hdl,
                                        OVERRIDE_EP_HANDLE spkr_hdl)
{
    return stream_connected_to_endpoints_have_sidetone_route((ENDPOINT*) mic_hdl,
                                                             (ENDPOINT*) spkr_hdl);
}

/* get_override_ep_ratematch_adjustment */
bool opmgr_override_set_ratematch_adjustment(OVERRIDE_EP_HANDLE ep_hdl,
                                             int value)
{
    return stream_configure_connected_to_endpoint((ENDPOINT*) ep_hdl,
                                                  OVERRIDE_RATEMATCH_ADJUSTMENT,
                                                  (uint32) value);
}

/* set_override_ep_set_hw_warp_apply_mode */
bool opmgr_override_set_direct_warp(OVERRIDE_EP_HANDLE ep_hdl, bool enable)
{
    return stream_configure_connected_to_endpoint((ENDPOINT*) ep_hdl,
                                                  OVERRIDE_HW_WARP_APPLY_MODE,
                                                  (uint32) enable);
}

/* get_overriden_endpoint*/
OVERRIDE_EP_HANDLE opmgr_override_get_endpoint(OPERATOR_DATA *op_data,
                                               unsigned int terminal_id)
{
   INT_OP_ID op_id = op_data->id;
   unsigned int opidep;

   opidep = get_opidep_from_opid_and_terminalid(op_id, terminal_id);
   return (OVERRIDE_EP_HANDLE) stream_endpoint_from_extern_id(opidep);
}
