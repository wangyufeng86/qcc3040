...
#if defined(SUPPORTS_MULTI_CORE)
#define INVALID_PROCESSOR_ID(pid) ipc_invalid_processor_id((PROC_ID_NUM)pid)
#else
...
#if defined(SUPPORTS_MULTI_CORE)
static bool kip_list_resp_handler(CONNECTION_LINK con_id,
                                 ...
                                 preproc_function preproc_func)
{
    ...
    if (PROC_PRIMARY_CONTEXT())
    {
        ...
        if (opcmd_context != NULL)
        {
            ...
            * consecutively.
            */
            con_id = PACK_CON_ID(GET_CON_ID_SEND_ID(con_id),
                                 (CONNECTION_PEER) opcmd_context->agid);
 ...
 *
 */
static void opmgr_create_operator_post_dnld(CONNECTION_LINK con_id,
                                            CAP_ID cap_id,
                                            unsigned int op_id,
                                            unsigned int priority,
                                            PROC_ID_NUM processor_id,
                                            ...
                                            OP_CREATE_CBACK callback)
{
     ...
     */
    if ((PROC_PRIMARY_CONTEXT() && INVALID_PROCESSOR_ID(processor_id)) ||
        (PROC_SECONDARY_CONTEXT() && ((!PROC_ON_SAME_CORE(processor_id)) || (op_id == 0))))
    {
...
#endif /* defined(SUPPORTS_MULTI_CORE) */
    {
        ...
        opmgr_tag_dm_memory(new_op, (DMPROFILING_OWNER)new_op->task_id);
...
static bool opmgr_create_operator_cback(STATUS_KYMERA status)
{
                              ...
                              OP_CREATE_CBACK callback)
{
    ...
    PROC_ID_NUM processor_id = PROC_PROCESSOR_0;
    unsigned int i;
    ...
    for(i=0; i<num_keys; i++)
    {
        switch(info[i].key)
        {
            ...
            case OPERATOR_CREATE_PROCESSOR_ID:
            {
                 ...
                 * the key. */
                if (!proc_is_valid((unsigned)info[i].value))
                {
                    callback(REVERSE_CONNECTION_ID(con_id), STATUS_CMD_FAILED, op_id);
                    return;
                }

                processor_id = (PROC_ID_NUM) info[i].value;

#if defined(SUPPORTS_MULTI_CORE)
                /* The primary processor is always available while auxiliary processors might not be. */
                if ((info[i].value != 0) && (!kip_aux_processor_has_started((PROC_ID_NUM)info[i].value)))
                {
#ifdef INSTALL_THREAD_OFFLOAD
...
#if defined(SUPPORTS_MULTI_CORE)
    if (PROC_PRIMARY_CONTEXT() && (processor_id != PROC_PROCESSOR_0))
    {
...
void opmgr_create_operator(CONNECTION_LINK con_id,
                           CAP_ID cap_id,
                           unsigned int op_id,
                           OP_CREATE_CBACK callback)
{
                                                  ...
                                                  unsigned err_code)
{
     ...
     */
    opmgr_destroy_operator(GET_CON_ID_RECV_ID(reversed_con_id), destroy_list[0], &destroy_list[1],
...
#if defined(SUPPORTS_MULTI_CORE)
    source_valid = (sender_con_id == owner_con_id) || (GET_SEND_RECV_ID_CLIENT_ID(GET_CON_ID_SEND_ID(sender_con_id)) == OPMGR_ADAPTOR_OBPM);
#ifdef INSTALL_AOV
    source_valid = source_valid ||
                   (GET_SEND_RECV_ID_CLIENT_ID(GET_CON_ID_SEND_ID(sender_con_id)) == AOVSM_ID);
#endif /* INSTALL_AOV */
#else
 ...
 * processor-specific - in which case con_id has to be accurately passing processor ID, too.
 */
void opmgr_get_capability_info(CONNECTION_LINK con_id,
                               unsigned capid,
