...
Used registers:                                      BC5 routine used:
   ...
   Stack (relative to SP at entry):
   [6 - SP points here within stage_loop]
   5 - Pointer to imaginary array, SP - ADDR_PER_WORD
   4 - Pointer to real array, SP - 2*ADDR_PER_WORD
   3 - I7 (FFT struc pointer)
   2 - next groups (1,2,...,Npts/4,Npts/2), FP + 2*ADDR_PER_WORD
   1 - rLink (return address)
   0 - old FP (new FP points here)
   (All *ADDR_PER_WORD)

   *TODO fix mixed use of SP and FP-relative offsets

*/

.CONST $fft.local.next_groups          2*ADDR_PER_WORD;
   ...
   pushm <FP(=SP), rLink>, SP = SP + ADDR_PER_WORD; // Reserve space for local variable next_groups
   ...
   stage_loop:
#ifdef KAL_ARCH3
      r0 = FP + $fft.local.next_groups;
      I1 = r0;
#else
      I1 = FP + $fft.local.next_groups;
#endif
   ...
   #ifdef FFT_PIPELINED_LAST_STAGES
      jump $math.private.scaleable_fft_penultimate_stage;
   #else
      jump $math.private.scaleable_fft_last_stage;
   #endif
.ENDMODULE;
...
//
// *****************************************************************************
.MODULE $M.math.private.scaleable_fft_penultimate_stage;
   .CODESEGMENT MATH_PM;
   .DATASEGMENT DM;

   $math.private.scaleable_fft_penultimate_stage:
...
//
// *****************************************************************************
.MODULE $M.math.private.scaleable_fft_last_stage;
   .CODESEGMENT MATH_PM;
   .DATASEGMENT DM;

   $math.private.scaleable_fft_last_stage:
      ...
      SP = SP - ADDR_PER_WORD, popm <FP, rLink>;
      
      rMAC = M[I7,-MK1]; // Decrement I7 back to where it was when the function was called
...
.MODULE $M.math.load_fft_twiddle_factors;
    // Potential candidate for being placed in FAST PM, optionally
   .CODESEGMENT MATH_OPTIONAL_FAST_PM;
   .DATASEGMENT DM;
