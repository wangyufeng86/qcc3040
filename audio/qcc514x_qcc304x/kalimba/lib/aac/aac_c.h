...
typedef struct
{
   ...
   /* tables */
    ...
    unsigned int use_huffman_tables_from_rom;
...
/**
 *    Create a dummy AAC decoder with a minimal set of tables initialised
 *    This will be used by libraries/capabilities to determine the number
 *    of audio samples contained in an AAC LC packet transported over LATM
 *    (utility to call: aacdec_samples_in_packet_lc).
 *
 *    \param none
 *    \return pointer to the dummy decoder's structure
 */
extern aac_codec * aacdec_create_dummy_decoder(void);

/**
 *    Destroy a dummy AAC decoder
 *
 *    \param pointer to the dummy decoder's structure
 *    \return none
 */
extern void aacdec_destroy_dummy_decoder(aac_codec *);
