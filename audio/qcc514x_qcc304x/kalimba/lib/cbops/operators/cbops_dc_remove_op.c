 ...
 * \ingroup cbops
 *
 * This file contains functions for the DC offset removal cbops operator
 */
...
cbops_op* create_dc_remove_op(unsigned nr_channels, unsigned* input_idx, unsigned* output_idx)
{
    ...
    /* Allocate for struct for all channels, parameters for one channel is included in the size of cbops_dc_remove */
    cbops_op* op =
        (cbops_op *)xpmalloc(sizeof_cbops_op(cbops_dc_remove, nr_channels, nr_channels) +
                             (nr_channels - 1)*sizeof(cbops_dc_remove));
    ...
    if(op != NULL)
    {
         ...
         * allocated after the cbop parameter struct, thus they follow each other after the
