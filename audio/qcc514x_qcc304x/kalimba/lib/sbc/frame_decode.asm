...
.MODULE $M.sbcdec.frame_decode;
   ...
   if EQ jump buffer_underflow;
      ...
      // If we've already consumed an octet before the syncword: remove it from available bytes
      
      // Get the bit position of the sync byte within a word
      // A bitpos of 0 indicates that we are at the end of the previous word
#ifdef DATAFORMAT_32
      r0 = 0x20 - r6;
#else
      r0 = 0x10 - r6;
#endif
      // Divide the bitpos by 8 to get octet position
      r0 = r0 LSHIFT -3;
      r10 = r10 - r0;
      // Add the syncbyte again (sbcdec.find_sync changes r10 by at least -1)
      r10 = r10 + 1;
      r4 = r4 - r10;
   ...
   if Z jump no_output_check_needed;
      ...
      if Z jump output_check_no_left_channel;
         ...
         if NEG jump not_enough_space_exit;
   ...
   // each word has 32-bits of coded data (four bytes) or 16-bits (two bytes)
#ifdef DATAFORMAT_32
