...
#ifdef DATAFORMAT_32
#define TMP_ENC_BUFF_SIZE                  15 /* keep in sync with $sbc.mem.TMP_ENC_BUFF_SIZE from sbc.h */
#define TMP_ENC_BUFF_TOTAL_SIZE            (TMP_ENC_BUFF_SIZE + sizeof(tCbuffer))
#define TMP_ENC_DEC_BUFF_SIZE              (TMP_ENC_BUFF_TOTAL_SIZE + sizeof(tCbuffer))
#define TMP_ENC_DEC_BUFF_TOTAL_SIZE        (TMP_ENC_DEC_BUFF_SIZE + sizeof(tCbuffer))
#define WBS_TOTAL_WORDS                    (7 + TMP_ENC_DEC_BUFF_TOTAL_SIZE)
/* 7 comes from the other 7 fields for WBS in sbc.h that are not buffers/cbuffer_strucs.
    (count of scalar fields after WBS_FRAME_BUFFER_PTR_FIELD) */
#endif /* DATAFORMAT_32 */
...
#ifdef DATAFORMAT_32
    /* The last 40 of the asm structure definition fields are for WBS only*/
    void *wbs_frame_buffer_ptr;
    unsigned fields4[WBS_TOTAL_WORDS];
#else
