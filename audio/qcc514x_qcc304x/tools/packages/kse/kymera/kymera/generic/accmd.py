...
class Accmd(object):
    ...
    def _message_received(self, msg):
        ...
        for cb in self._cb:
            if not cb[FILTER] or cb[FILTER](msg):
                cb[CALLBACK]([IntHexU16(entry) for entry in msg])
    ...
    @log_input_output(logging.INFO, logging.INFO, formatters={'cap_id': '0x%04x'})
    @log_exception
    ...
    def get_mips_usage(self, operators):
        ...
        # TODO this is the right check if len(payload) < 2 or 2 * payload[1] != (len(payload) - 2):
    ...
    @log_input_output(logging.INFO, logging.INFO, formatters={'source_id': '0x%04x'})
    @log_exception
    ...
    @log_input_output(logging.INFO, logging.INFO, formatters={'sink_id': '0x%04x'})
    @log_exception
    ...
    def clock_configure(self, low_power_mode_clk, custom_mode_clk, active_mode_clk):
        '''
        This command allow the APP to register its clock preference for the various operation modes.
