...
def log_input(level=logging.DEBUG, formatters=None, ignore=None):  # @DontTrace
    ...
    def wrapper(wrapped, instance, args, kwargs):  # @DontTrace
        ...
        Args:
            ...
            kwargs (dict): Keyword arguments supplied when the decorated function was called.
        '''
        args_spec = inspect.getfullargspec(wrapped)  # pylint: disable=no-member,deprecated-method
        ...
        args_key_vals = [
            (_get_repr(k, v), _get_repr_value(v, formatter=formatters.get(k, '%s')))
            for k, v in zip(arg_names, args)
            if k not in ignore
        ]
        args_str = ' '.join(
            '{k}:{v}'.format(k=k, v=v) for k, v in args_key_vals
        )

        kwargs_key_vals = [
            (_get_repr(k, v), _get_repr_value(v, formatter=formatters.get(k, '%s')))
            for k, v in kwargs.items() if k not in ignore
        ]
        kwargs_str = ' '.join(
            '{k}:{v}'.format(k=k, v=v) for k, v in kwargs_key_vals
        )
...
def log_input_output(in_level=logging.DEBUG, out_level=logging.DEBUG,
    ...
    def wrapper(wrapped, instance, args, kwargs):  # @DontTrace
        ...
        # pylint: disable=protected-access
        param_str = _get_param(instance)
        instance._log.log(in_level, '%s%s %s', param_str, wrapped.__name__, all_arguments)
